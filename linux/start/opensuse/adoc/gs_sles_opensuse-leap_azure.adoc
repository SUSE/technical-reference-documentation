...
// document
// -
:title: Migrate to SUSE Linux Enterprise from openSUSE Leap on Microsoft Azure
:subtitle: Enterprise Cloud Computing

:product1: SUSE Linux Enterprise
:product1_short: SLE
:product1_version: 15
...
:product3: Microsoft Azure
:product3_short: Azure
:product3_url: https://azure.microsoft.com/en-us

:usecase: migrate to supported, pay-as-you-go {product1} from {product2} on {product3}

:executive_summary: Learn how to {usecase}.
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
Several years ago, SUSE and the openSUSE community began working to bring the source code streams of {product1} ({product1}) and {product2} into alignment.
The goal was bring the experience and quality of both products to a new level.
...
With {product2} {product2_version_min} and {product1} {product1_version_min}, it became possible to migrate from the community Linux to supported, enterprise Linux.
Migration became even simpler with later releases.
You can even move from an {product2} image in the cloud to a supported, {product1}, pay-as-you-go (PAYG) image, using the {product3} Hybrid Benefit.
...
Following step-by-step guidance, you will migrate from {product2} to {product1} on {product3}.
...
Developers and administrators, who may have started with {product2} on {product3} for development, can use this guide to migrate to a supported, {product1} deployment for production.
...
To perform the steps described in this guide, you need:

* {product3} account

* {product2} {product2_version_min} or later, running in {product3}

* {product1} Server subscription
...
The process to switch from {product2} to {product1} Server in {product3} is outlined below.
...
The general procedure you will follow consists of the following steps:

. Install `yast2-registration-sle` in your {product2} system.

. Register the {product2} system with a valid subscription.

. Reboot.
//
+
At this point, you have a virtual machine running {product1} Server.

. Install te Azure Hybrid Benefit extension to switch to PAYG.
...
PAYG:: is a pricing option where you pay Microsoft for the resources you use on a time based usage, e.g. and hourly or monthly. You only pay for what you have used. It includes the OS and the used infrastructure.
...
Depending on your needs and requirements you need to decide to run you future SLES
as PAYG or BYOS
...
For the example here we use openSUSE Leap 15.4 as starting with this version it is simpler as before.

* start with a openSUSE image in the Azure Marketplace
      for example opensuse-leap-15-4:gen2:2022.11.04
...
* get a 60d-trial subscription for SLES via download.suse.com to use the *PAYG* version of SLES
* or
...
IMPORTANT: This yast2 process ask you for your email and the subscription key

* Now you have a SLES BYOS instance. Check it with
[source]
      cat /etc/os-release

NOTE: The openSUSE repositories remain attached and are handled as third party repositories at this point and not get touched.
