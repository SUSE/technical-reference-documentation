...
INFO: The project uses PAYG images by default. This means there is no need for subscriptions or license files to provide for an easy start. 
...
ifeval::[ "{cloud}" != "GCP" ]
Here in the example, we use the compressed archives for the installataion (exe/RAR) as this is the easiest method to download and upload to the cloud share.
