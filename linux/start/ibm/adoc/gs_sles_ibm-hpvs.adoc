...
The Crypto Passthrough capability enables Hardware Security Modules (HSM) to be available to the IBM Hyper Protect Container Runtime (HPCR) where the containerized workload will run. A https://youtu.be/f7iQ-1dE9Ag?feature=shared[video] is available which explains crypto passthrough with a demonstration application built on a SUSE Linux Enterprise Base Container image.
...
[NOTE]
====
This releases all domains from host control.  For information about handling domains more selectively, see https://www.ibm.com/docs/en/linuxonibm/com.ibm.linux.z.lkdd/lkdd_t_crypto_wrk_mask.html[Freeing AP queues for KVM guests].
====
...
. Execute the following commands to associate the domain as a _vfio_ap_ controlled device.
+
[source, console]
----
uuid=$(uuidgen)
...
. Verify the card and domain are controlled by _vfio_ap_.
+
[source, console]
----
CARD.DOM TYPE  MODE        STATUS     REQUESTS  PENDING HWTYPE QDEPTH FUNCTIONS  DRIVER
...
The https://registry.suse.com/repositories/bci-nodejs22?arch=s390x[SLE BCI Language Container Images] for Node.js development is used as base for the PayNow Node.js application container image.
