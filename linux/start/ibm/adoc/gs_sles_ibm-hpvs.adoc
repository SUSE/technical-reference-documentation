...
--network type=direct,source=eth0,source.mode=bridge,model.type=virtio,driver.name=vhost,mac.address=<macaddress> \
...
----
+
where <ip address> is the IP address of the HPCR VM that will run the PayNow website application.
...
[NOTE]
====
Replace _<github user>_ with your personal GitHub username.
====
...
target.bus=virtio \
  ...
  --network type=direct,source=eth0,source.mode=bridge,model.type=virtio,driver.name=vhost,driver.iommu=on,mac.address=<macaddress> \
  --console pty,target.type=sclp,target.port=0 \
  --audio id=1,type=none \
  --memballoon none \
  --panic s390 \
...
+
.. Log into the rsyslog server with `ssh`.
+
...
The attestation data creation process is the same in https://cloud.ibm.com/docs/vpc?topic=vpc-about-attestation[IBM Cloud] and https://www.ibm.com/docs/en/hpvs/2.1.x?topic=servers-attestation[on-premises] storing several attestation-related files in `/var/hyperprotect` on the container runtime.
The attestation record (also referred to as the https://cloud.ibm.com/docs/vpc?topic=vpc-about-attestation#attestation_doc[attestation document]) contains checksums, which are used to verify the integrity of the environment where a workload starts.
...
[source, console]
----
openssl genpkey -aes256 -algorithm RSA -pkeyopt rsa_keygen_bits:2048 -out attest-private-key.pem
openssl pkey -in attest-private-key.pem -out attest-public-key.pem -pubout
----
+
[NOTE]
====
You will be asked to define a passphrase when creating the private key as well as provide the same passphrase when creating public portion of the key. 
====
...
. Save the encrypted attestation document to `se-checksums.txt.enc`.
//
+
...
..  Generate a SHA256 checksum of the local contract file.
+
[source, console]
----
...
.. Compare the output of the above command with the `user-data` line in the `se-checksums.txt` attestation document.
//
+
...
This guide featured the PayNow Website application.
...
* https://documentation.suse.com/trd/linux/tbd[Build] a trusted PayNow Website container on {product1_full} using https://cloud.ibm.com/docs/vpc?topic=vpc-about-hpsb[{hpp} Secure Build].
