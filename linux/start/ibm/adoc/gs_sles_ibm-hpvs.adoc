...
The manage section focuses on steps in the <<Workflow>> that introduce additional ways to interact with the confidential containized workload.

=== Enable and verify attestation

The IBM HyperProtect Platform container runtime generates attestation data as it starts.
The attestation data creation process is the same in https://cloud.ibm.com/docs/vpc?topic=vpc-about-attestation[IBM Cloud] and https://www.ibm.com/docs/en/hpvs/2.1.x?topic=servers-attestation[on-premises] storing the data in `/var/hyperprotect` on the container runtime.
Several attestation related files are stored in `/var/hyperprotect`.
The attestation record, also referred to as the https://cloud.ibm.com/docs/vpc?topic=vpc-about-attestation#attestation_doc[attestation document], contains checksums which are used to verify the integrityof the environment where a workload starts.
...
For example if you are using the pre-built PayNow Node.js application container image.
+
[source, yaml]
----
services:
...
[NOTE]
====
You will be asked to define a pass phrase when creating the private key as well as provide the same pass phrase when creating public portion of the key. 
====
+
[source, console]
----
...
.. Add `attestationPublicKey` to the contract.
+
[NOTE]
====
A portion of the contract file is shown.
====
+
[source, yaml]
----
...
----
+
where the output of the following command replaces the <attest-pubkey>.
+
`IFS=$'\n'; echo -n \"; for line in $(cat attest-public-key.pem); do echo -n $line"\n"; done; echo \"`

. Deploy the workload either in IBM Cloud or on-premises.

. Enter https://<ip address>:8443//api/v1/attestation to access the encrypted attestation document.
//
+
The floating IP address noted earlier for the VPC deployment in IBM Cloud.
+
...
//
+
The file can be save on the local system where the Docker Compose file is located.
