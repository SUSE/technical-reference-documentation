...
** https://www.ibm.com/docs/en/hpvs/2.1.x?topic=servers-crypto-passthrough#hpcr_prerequisites[IBM z16 or IBM LinuxONE 4] is required
** The Host Key Document (HKD) for the IBM Z or IBM LinuxONE server which was obtained in an earlier step.
** One or more https://www.ibm.com/docs/en/linux-on-systems?topic=execution-crypto-express-adapters#lxse_ap_pass__title__2[supported and properly configured IBM Crypto Express adapters]
//
+
...
* <<Prepare IBM Crypto Express adapters and domains to be passed into a containerized workload>>
...
* <<Adding IBM Crypto Express adapters and domains to the contract>>
...
* <<On-premises deployment>>
* <<Adding IBM Crypto Express adapters and domains to the deployment>>
...
. Select __Manage__ > __Access (IAM)__ in the drop-down box in the middle of the IBM Cloud page.
+
[NOTE]
====
...
. Select the Cloud Logs instance created.
...
The https://www.ibm.com/docs/en/linux-on-systems?topic=recommendations-kvm-host-networking-configuration-choices[KVM Host Networking Configuration Choices] 
in the IBM documentation provides a detailed list of networking choices and pros and cons.
...
Confirm prerequisites like https://www.ibm.com/docs/en/linux-on-systems?topic=choices-osa-interface-traffic-forwarding[OSA interface traffic forwarding] 
are met for your choice of networking.

This getting started guide will use the https://www.ibm.com/docs/en/linux-on-systems?topic=choices-using-macvtap-driver[MacVTap driver] 
in Bridge mode with the `eth0` interface.  
...
For example, `ibm-hyper-protect-container-runtime-23.3.0.qcow2` is the file name which is located in directory `/opt/hpvs-hpcr/IBM-HPVS-OnPrem-v2.1.4-EN-Trial/images`.

==== Setting up an `rsyslog` logging service for HPVS log information

With the release of SUSE Linux Enterprise Server 15 SP5, Minimal VM images for IBM Z and LinuxONE systems can be downloaded from https://www.suse.com/download/sles/[suse.com].
Two KVM `qcow2` image file options are available: _kvm_ and _Cloud_.
...
. Log in to the VM via SSH as the _sles_ user from another system using the private portion of the SSH key defined above.
...
----
+
=> where logging information will be written to `/var/log/hpcr.log`.
...
The Crypto Passthrough capability enables Hardware Security Modules (HSM) to be available to the IBM Hyper Protect Container Runtime (HPCR) 
where the containerized workload will run. A https://youtu.be/f7iQ-1dE9Ag?feature=shared[video] is available which explains crypto passthrough 
with a demonstration application built on a SUSE Linux Enterprise Base Container image.

To enable virtual cryptographic adapters to be passed through, specific domains also referred to as AP queues must have an 
association request created.

[IMPORTANT]
====
The domain associations that are created following these steps do not persist after a reboot of the KVM host. 
For information about creating a persistent association, see https://www.ibm.com/docs/en/linux-on-systems?topic=through-ap-queues[Preparing pass-through devices for cryptographic adapter resources].
====

. Use `lszcrypt -V` to review the Crypto Express cards and domains that are available to be associated.
+
[NOTE]
====
...
--------------------------------------------------------------------------------------------
00       CEX8P EP11-Coproc online            3        0     14     08 -----XN-F- cex4card
00.0032  CEX8P EP11-Coproc online            3        0     14     08 -----XN-F- cex4queue
----

. Make note of the Master Key Verification Pattern (mkvp) which will be used in following sections. [[note-mkvp]]
+
[source, console]
----
cat /sys/devices/ap/card00/00.0032/mkvps
----

. Release the domains from the direct control by the host operating system.
+
[source, console]
----
...
[NOTE]
====
This releases all domains from host control. For information about handling domains more selectively, 
see https://www.ibm.com/docs/en/linuxonibm/com.ibm.linux.z.lkdd/lkdd_t_crypto_wrk_mask.html[Freeing AP queues for KVM guests].
====

. Load the virtual function IO adapater ap kernel module.
+
[source, console]
----
modprobe vfio_ap
...
. Execute the following commands to associate the domain as a `vfio_ap` controlled device.
+
[source, console]
----
uuid=$(uuidgen)
...
. Verify the card and domain are controlled by `vfio_ap`.
+
[source, console]
----
CARD.DOM TYPE  MODE        STATUS     REQUESTS  PENDING HWTYPE QDEPTH FUNCTIONS  DRIVER
--------------------------------------------------------------------------------------------
00       CEX8P EP11-Coproc online            4        0     14     08 -----XN-F- cex4card
00.0032  CEX8P EP11-Coproc assigned          -        -     14     08 -----XN-F- vfio_ap
----
...
The steps outlined here are the same for cloud and on-premises deployments.

You will use the https://cloud.ibm.com/docs/vpc?topic=vpc-financial-transaction-confidential-computing-on-hyper-protect-virtual-server-for-vpc[`PayNow Node.js` application], 
which has been already built on a SLE BCI container image and published to a container registry.
...
An option is to use an IBM Cloud virtual server instance (VSI) for VPC with a SUSE Linux Enterprise Server https://cloud.ibm.com/docs/vpc?topic=vpc-vsabout-images#vs-s390x-supported-os[s390x stock virtual server image]
but any Linux distribution on s390x with `git` and `podman` is sufficient.

The https://registry.suse.com/repositories/bci-nodejs22?arch=s390x[SLE BCI Language Container Images] for `Node.js` 
development is used as base for the `PayNow Node.js` application container image.
...
The `PayNow Node.js` application container image has already been built on a SLE BCI container and published to a container registry.
You can go to <<Defining the contract>> to use the `PayNow Node.js` application container image or follow the steps below 
to publish your own image of the `PayNow Node.js` application.
...
[IMPORTANT]
====
The https://www.ibm.com/docs/en/hpvs/2.2.x?topic=servers-crypto-passthrough#hpcr_sample_crypto_passthrough_contract[IBM Crypto Passthrough documentation] provides options - `secret` and `pvsecret` - to pass the crypto device details within the contract. This demo is using the `secret` option, for simplification. The HPCR bootstrap will transparently execute the `pvsecret` using IBM validated and curated code.  Using the `pvsecret` option requires additional effort by a relying party persona. Using `pvsecret` provides additional security because the attestation request and result are created and validated by the relying party rather than by IBM provided bootstrap code.
====

. Add one or more _index_ sections in the _crypto-pt_ section to the _env_ section in the contract.
+
[source, yaml]
----
env: |
...
----
+
=> where _lock:_ is `true`.
+
=> where <num> in _index-<num>:_ must start at 1 and must increase in sequence for each domain-id passed via the contract. For example, 1, 2, 3, and so on.
+
=> where _type:_ is `secret`.
+
=> where _domain-id:_ is the '<card>.<domain>' of the device being passed.
+
=> where _secret:_ can be generated with `dd if=/dev/random bs=32 count=1 2> /dev/null | base64`.
+
=> where _mkvp:_ was noted in the <<note-mkvp>> step.

. Add one or more _HKD-<model>-<plant><serial>_ sections in the _host-attestation_ section to the _env_ section in the contract.
+
[source, yaml]
----
env: |
...
----
+
=> where <model>, <plant> and <serial> in _HKD-<model>-<plant><serial>:_ follows the naming convention of the download HKD file.
+
=> where _description: is a description of the HKD.
+
=> where _host-key-doc:_ can be generated with `IFS=$'\n'; echo -n \"; for line in $(cat <path to downloaded HKD document> | base64); do echo -n $line"\n"; done; echo \"`

An example of the _env_ section of the contract with only the _crypto-pt_ and _host-attestation_ sections is provided below.
...
env: |
  ...
  host-attestation:
    HKD-3931-0271CC8:
      description: zt14
      host-key-doc: "LS0tLS1CRUdJTiBDRVJUSUZJQ0FURS0tLS0tDQpNSUlGSmpDQ0F3NmdBd0lCQWdJSlVVSG...NCi0tLS0tRU5EIENFUlRJRklDQVRFLS0tLS0NCg==\n"
----
...
. Define and start the `slebci-paynow-website` VM. [[define-start-vm]]
+
[source, console]
----
virt-install \
...
. Follow all of the steps in the previous section returning here when you get to the <<define-start-vm>> step.

. Determine and make note of the name of the crypto domain node device.
+
[source, console]
----
...
This section focuses on steps in the <<Workflow>> that introduce additional ways to interact with the confidential containerized workload.
...
* Build a trusted PayNow Web site container on {product1_full} using https://cloud.ibm.com/docs/vpc?topic=vpc-about-hpsb[{hpp} Secure Build].
