...
The Crypto Passthrough capability enables Hardware Security Modules (HSM) to be available to the IBM Hyper Protect Container Runtime (HPCR) where the containerized workload will run. A https://www.youtube.com/suse[video] is available which explains crypto passthrough with a demonstration application built on a SUSE Linux Enterprise Base Container image.
