...
An application container image is built by adding the application with the libraries and other software it needs to run (its dependencies) as layers on top of a base container image.
...
// Outline what is required, then walk through the steps to create the file
