...
* `uidmap` and `gidmap` specify user and group mappings, respectively.
//
+
Recall that you specified the UID and GID of the jupyter user in your _Containerfile_.

* `--name` to set a name for the container.

* `--publish` or `-p` to map a container's network port or range of ports to the host.
...
[NOTE]
====
If SELinux is enabled on your host system, Podman can help you properly label the mounted volume.
Do this by appending `:z` or `:Z` to the volume option.
That is,

* `--volume $(pwd):/home/jupyter/notebooks:z` if one or more containers will share the volume contents.

* `--volume $(pwd):/home/jupyter/notebooks:Z` if you only want the current container to use the volume.
...
[TIP]
====
Rather than typing this command every time you want to launch your container, create a shell script to do it.
 
. Create the text file _runjup_ with this content.
+
[source, bash]
----
#!/bin/bash
...
. Grant your user execute permissions for this script.
+
[source, bash]
----
chmod u+x runjup
