...
[listing]
----
# specify a base container image
# see: https://registry.suse.com/
FROM registry.suse.com/bci/bci-base:latest

# ensure zypper can run without user interaction
ENV ZYPPERCMD="zypper --non-interactive"

# install base Python components from the OS repositories
RUN $ZYPPERCMD install python3 python3-pip

# create a Python virtual environment in the /home/virtenv directory
RUN python3 -m venv /home/virtenv

# activate the Python virtual environment and upgrade pip
...
# run entrypoint.sh script to map user permissions
...
[TIP]
====
You can also list images with the command `podman image list`.
Learn more about Podman commands at: https://docs.podman.io/en/latest/Commands.html
====

Before running your container, add a location for the MNIST code and pass your User ID.
...
When the _podman run_ command starts your container, you will see its output in your shell.
It will look something like this:

. Example output of the _podman run_ command
+
----
sles@sp4:~/jupcontainer$ podman run --network host --env LOCAL_USERID=$UID localhost/jupyter
