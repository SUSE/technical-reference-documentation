...
You can set this without administrator help by creating the file _$HOME/.config/container/storage.conf_ with the following content:
...
The container image you have built now lives in your localhost repository and is tagged with the name _jupyter_.
...
----
Note your container image is named, _localhost/jupyter_.
+
[TIP]
====
...
podman run --name jupyter \
 ...
 -v $HOME/jupyter/jup2/data:/home/jupyter/data \
 localhost/jup2
----
+
...
//
+
Copy and paste the last URL (i.e., the one that begins with 'http://127.0.0.1') to your Web browser on your host system.
+
// put screenshot here

. Verify file access.
// Use screenshots to show creating a text file in JupyterLab, then show long directory listing with proper file ownership
+
[source, bash]
----
ls -l
...
[source, bash]
----
terry@tsleh1:~/code/jup2/data> ls -l
total 4
-rw-r--r-- 1 sarah users 66 Nov  8 18:09 test-cont.txt
-rw-r--r-- 1 sarah users  0 Nov  8 16:52 test-host
----

[TIP]
====
When you are done with your container, use __File__ > __Shutdown__ in the JupyterLab interface or issue Ctrl-C in the terminal where you started the container.A
====
