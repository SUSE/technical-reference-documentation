...
//
+
This guide leverages {productname} SP{servicepack} as a host operating system for building and running containers.
...
[NOTE]
====
If you modify either _/etc/subuid_ or _/etc/subgid_, you must stop all running containers owned by the user and terminate the pause process running for that user.
...
This is done system-wide by specifying the location with the 'rootless_storage_path' setting in  the '[storage]' section of _/etc/containers/storage.conf_.
...
Thus, you do not need to grant your user access to your SCC credentials for this guide.
...
This does not impact your ability to build containers unless you need to install official RPM packages.
====

To install official RPM packages from the {productname} repositories, you need to grant read permission for your current user with:
...
Complex applications, like an ML model, can produce wildly different results or not run because of minor code variations in a library dependency.
...
Your _requirements.txt_ file is a simple list of the names and versions of required packages.
Each package you specify may have several dependencies of its own, and you can explicitly list every package and its version in your _requirements.txt_ file.
...
You can overcome this is by removing the version specification (for example, specify `seaborn` instead of `seaborn==0.12.1`).
This frees the `pip` dependency resolver to substitute in an appropriate, available version of the package.

[WARNING]
====
Unexpected behaviors can arise because of code changes in major packages or any of their dependencies.
====


[TIP]
====
When you have verified a functioning configuration, you can automatically generate an updated _requirements.txt_ file.
...
For this guide, you use the 'latest' {productname2} Python container image, which, at the time of this writing, is built on {productname} SP{servicepack} and features Python {python_ver}.
...
See the https://docs.podman.io/en/v4.3/markdown/podman-run.1.html[Podman run documentation] for a more detailed discussion.
+
[NOTE]
====
...
To use this option, replace the `--uidmap` and `--gidmap` options in your launch script as follows:
...
To list all images, use `podman image list --all`.
====
...
A couple options are outlined here.
...
The recipient can use the received files and follow the same steps you used to create the container image and run the container.

. Use an image archive.
//
+
Use https://docs.podman.io/en/latest/markdown/podman-save.1.html[podman image save] to save your container image and its parent layers to a TAR archive that you can share.
The recipient of the TAR archive can load the image with https://docs.podman.io/en/latest/markdown/podman-load.1.html[podman image load] and run it with `podman run`.
+
[TIP]
====
...
//
+
Podman provides the https://docs.podman.io/en/v4.3/markdown/podman-push.1.html[podman push] command for publishing your container images to a registry and the https://docs.podman.io/en/v4.3/markdown/podman-pull.1.html[podman pull] command for retrieving images from a registry.
+
====
...
They enable operations teams to manage the full life cycle of complex application landscapes across a broad spectrum of infrastructure and services - in the data center, in the cloud, and at the edge.
Innovation across many domains, including machine learning, has been accelerated by the availability of container technologies that make it easier to collaborate, test, and deploy new capabilities.


In this guide, you took first steps working with containers.
You learned how to install Podman, then use it to build, run, manage, and share an application container.
You also learned how to use {platform2} as a reliable, secure, supportable, and freely redistributable base layer for your containers.
