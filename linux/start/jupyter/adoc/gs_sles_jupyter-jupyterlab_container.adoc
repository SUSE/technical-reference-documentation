...
This step is optional, but it is covered here for completeness.

To install RPM packages from the {productname} repositories, your user account needs read access to your SUSE Customer Center credentials.
These are located in _/etc/zypp/credentials.d_.

For this guide, you use the {productname2} Python container image.
This container image already includes the development and runtime components for {otherproduct2}, saving you from needing to install them from the {productname} repositories.
If you choose to use a different {productname2} or wish to install other official RPM packages, you can grant read permission to your current user with:
