...
When you shut down JupyterLab, you also shut down the container.
This did not delete the container.
It still exists and you can restart and stop it again and again.
...
. Select __File__ > __Log Out__ and close your Web browser.
//
+
By using the __Log Out__ option, the JupyterLab server continues to run.
...
----
+
You have already used `podman image list` to list your existing images.  Synonyms for this command are `podman image ls` and `podman images`.
//
+
This command has options for formatting, filtering, and sorting image information.
