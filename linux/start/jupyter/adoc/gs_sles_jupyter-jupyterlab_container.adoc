...
Podman provides the https://docs.podman.io/en/v4.3/markdown/podman-push.1.html[podman push] command for publishing your container images to a registry and the https://docs.podman.io/en/v4.3/markdown/podman-pull.1.html[podman pull] command for retrieving an image from a registry (this is what the 'FROM' instruction in your _Containerfile_ does).
...
You can host your own container registry or use a commercial container registry.
...
* Ease of migration: How difficult would it be if you need to migrate to a different registry?
