...
Repeatability is critical in science and engineering.
...
[NOTE]
====
Each of these packages may have several dependencies.
In practice, a complete list of required packages can be quite long and may be explicitly included in the `requirements.txt` file for true replicability.
====
...
You can install the packages specified in a requirements.txt file with the command: `pip install -r requirements.txt`.
====


=== Create user ID mapping

Containers running in unprivileged (rootless) mode run in the user namespace.
Earlier, you defined subUID and subGID ranges, which are assigned to files and processes inside a running container and which are offset from the UIDs and GIDs of the host.
This means that if you mount a host filesystem to a container, any files created will be owned by one of these subUIDs and subGIDs.
So, your re
A
Inside the running container, your 
One reason an IT organization might prefer Podman is that it can run containers with requiring root level access.
Podman does this is by using the Linux kernel feature called namespaces.
User environments inside a namespace can be grouped and segmented away from the host's users.
This means your container can run as a different user ID on a target machine than, for example, your regular user ID.
This is a great feature, but it can be problematic in a development scenario, where you may wish to access files in your home directory.

To get around this difficulty, you can run a script to create a user in the container with the desired user ID.
