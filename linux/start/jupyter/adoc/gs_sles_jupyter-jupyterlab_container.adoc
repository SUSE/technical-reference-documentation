...
All additions and changes specified with the instructions in your _Containerfile_ are layered on top of a base layer.
The `FROM` instruction specifies which base layer to use.
In this case, the 'latest' https://documentation.suse.com/container/all/single-html/SLES-container/#cha-bci[base container image] in the https://registry.suse.com/[SUSE container registry] is to be used.

[NOTE]
====
JupyterLab is a Web application that uses TCP port 8888 for user connections.
The `EXPOSE` instruction informs the container engine that the container listens on the specified network port.
====
