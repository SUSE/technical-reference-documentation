...
Podman can be run as an ordinary, unprivileged user.
To enable this, you need to make a few changes to your host system.

. Define subUID and subGID ranges for your current user.
//
+
You can map the users running inside your container with users on your host system by specifying subordinate UIDs and GIDs.
These subUIDs and subGIDs are specified in _/etc/subuid_ and _/etc/subgid_, respectively.

.. Check the _/etc/subuid_ and _/etc/subgid_ files to see if these ranges are already defined for your user.
//
+
If they are, you can move on to grant your user access to Zypper credentials.

.. Add subUID and subGID ranges to your user.
+
[source, bash]
----
...
[WARNING]
====
The subUID and subGID ranges ('100000-165535' in the command example) may not overlap and must be different for each user on the host.
====
...
. Grant your user access to Zypper credentials.
//
+
