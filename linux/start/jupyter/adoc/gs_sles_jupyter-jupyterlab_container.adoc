...
This resource enables them to expand capabilities and accelerate development to address real-world challenges and use cases.

The rapid pace of innovation in AI/ML can also be challenging.
Complex applications often rely on functions and structures located in multiple, rapidly changing libraries.
As the underlying libraries change, the dependent applications can break or produce unexpected results.
To attempt to address this, many AI/ML applications require specific lbirary versions.
However, in multi-application and multi-user environments, it can be difficult or impossible to manage this software landscape through traditional methods.

Today, developers have access to container technologies to help overcome this challenge.
Containers provide a convenient, lightweight way to package applications and their dependencies while isolating these from other applications and their dependencies.
...
This document illustrates how to containerize and deploy {jup}, the {python} development platform for data analytics and AI/ML, using {bci-full} and {podman}.


=== Audience

This document is intended for AI/ML developers, researchers, operations teams, systems architects, and others interested in learning more about software development and deployment with container technologies.
To progress with this document, the reader should have basic Linux command line skills and familiarity with container concepts.
...
{jup-website}[JupyterLab] is an interactive development environment that is popular among machine learning, data science, and other researchers and developers.
...
{bci-full} are freely available, re-distributable, supported across many environments.
...
[TIP]
====
SUSE maintains up-to-date, ready-to-use container images at {bci-registry}.
====
