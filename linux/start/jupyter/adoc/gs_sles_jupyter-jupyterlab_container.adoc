...
// -
:title: Replicable Python Development Environments with JupyterLab
:subtitle: Using Podman SUSE Linux Enterprise Base Container Images
...
:usecase: replicable Python development environments with containers
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
Podman can be run as an ordinary, unprivileged user, but you may need to make a few configuration changes.
...
If they are not defined, you can do so with the following steps:

.. Update your user account with the required ranges.
+
[source, bash]
----
...
[WARNING]
====
The subUID and subGID ranges ('100000-165535' in the command example) must not reuse UIDs and GIDs of existing users or overlap for different users.
====
...
[NOTE]
====
Podman uses a pause process to keep the unprivileged namespaces alive.
To propogate subUID and subGID changes, you may also need to issue:
[source, bash]
----
...
* `--userns=keep-id` to map the container user to the same UID as your user on the host.
====
//
+
