...
The instructions used in this _Containerfile_ to automate the build of your container include:

* `FROM`: specifies which base image to use.

* `RUN`: executes a shell (`/bin/sh -c`) command.
...
* `CMD` provides a default command to be executed when the container is run.
...
Now that you have a container, you can run it with the `podman run` command.
But, for your application container to be useful, you need to enable access.
That is, you need to be able to access the JupyterLab Web interface (listening on TCP network port 8888) from your local host as well as to read and write files outside the container (such as on your local file system).
You do this with a few `podman run` options.

In this section, you create a shell script to simplify calling `podman run` with all required options.

. Change to your preferred directory for personal shell scripts (such as _$HOME/bin_) or temporarily use your build directory (_$HOME/jupcontainer_).
+
[source, bash]
----
cd $HOME/jupcontainer
...
podman run \
  --uidmap $container_user:0:1 \
  --uidmap 0:1:$container_user \
  --gidmap $container_group:0:1 \
  --gidmap 0:1:$container_group \
  --name $container_name \
...
. Grant yourself execute permissions on the script.
+
[source, bash]
----
chmod u+x juplaunch
...
Illustrated in the _juplaunch_ script are the following options:

`--name`:: Assigns a name to the container.

`--publish`:: Publishes the container's exposed network port or range of ports to the host, making it accessible.

`--volume`:: Creates a bind mount, mounting the specified volume from the host into to container.
+
[NOTE]
====
...
`--uidmap` and `--gidmap`:: Map the UID and GID, respectively, from the user namespace inside the container to your user namespace outside the container.
This allows you to avoid permission and ownership issues when accessing files in the mounted volume.
You do this with the `podman run` command options `--uidmap` and `--gidmap` (see the https://docs.podman.io/en/v3.4.4/markdown/podman-run.1.html[Podman run documentation] for a more detailed discussion).

[NOTE]
====
User namespace mappings can be complex, as rootless (unpriviledged) user UIDs and GIDs are mapped through intermediary (subordinate) UIDs and GIDs.

With the release of https://docs.podman.io/en/v4.3/markdown/podman-run.1.html[version 4.3.0], `podman run` simplifies user namespace mappings with the `--userns` option.
====



== Launch your application container
...
There you can find the URL and access token for the containerized JupyterLab environment.
+
[source, bash]
----
...
====
+
At this point, your container is started, but you do not have the proper URL of the JupyterLab server to access it.
You can obtain the URL from the command line inside the container.
...
----
Once you have this URL, you can exit the container shell and return to your local command line.

. Access JupyterLab by opening the displayed URL in your Web browser.
...
You can relaunch your Web browser and open the same URL to access JupyterLab.
====

. Shut down JupyterLab by selecting __File__ > __Shutdown__.
+
[TIP]
====
You can also shut down the JupyterLab server from the shell inside your container by issuing the `jupyter-server stop` command.
Once JupyterLab shuts down, you are returned to your local command line and the container is stopped.

You can also unceremoniously shut down the container with `podman stop CONTAINER-ID`.
====


== Clean your container environment

As you work with containers, you may find that you no longer need some of them.
Podman provides commands to help you clean up your container space.
...
//
+
You may wish to do this if, for example, you want to rebuild your application container image with newer packages.
...
Many Podman commands also have synonyms that can save you a few keystrokes (for example, `podman rmi IMAGE-ID` is the same as `podman image rm IMAGE-ID`).
