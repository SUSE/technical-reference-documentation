...
[TIP]
====
You can also use the aliases, `podman save` and `podman load`.
====
...
//
+
A container registry hosts respositories for storing and accessing container images.
With https://docs.podman.io/en/v4.3/markdown/podman-push.1.html[podman push] you can publish your image to a registry and with https://docs.podman.io/en/v4.3/markdown/podman-pull.1.html[podman pull] you can retrieve an image from a registry (this is what the 'FROM' instruction in your _Containerfile_ does).
//
+
Registries can be public or private.
Private registries are often used by organizations for internal or restricted sharing of container images.
Public registries typically offer open access to "pull" images, though registration and other requirements may exist for "pushing" images to the registry.


These are just a few of the options you can use to share your application container with others.
