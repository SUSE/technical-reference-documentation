...
The `podman build` command can build a container image from a set of instructions provided in a https://github.com/containers/common/blob/main/docs/Containerfile.5.md[_Containerfile_].
...
The _Containerfile_ consists of a sequence of instructions to automate the image build process.
The instructions used here are:
...
//
+
If you do not have your own Jupyter notebook to use, you can find a plethora to download from the Web, including Alexis Cook's excellent https://www.kaggle.com/code/alexisbcook/titanic-tutorial/notebook[Titanic Tutorial] for https://kaggle.com/[Kaggle].
...
Double-click a filename to open the file in a tab in the main pane.
+
...
Each cell may be one of three types:
...
Earlier in this guide, you used `podman build` to create a container image and `podman run` to instantiate the image as a container.

Explore some Podman commands in https://documentation.suse.com/sle-micro/5.3/html/SLE-Micro-all/article-podman.html#sec-working-containers[Podman Guide: Working with containers].


== Summary

Enterprises of all sizes recognize the need to transform the way software and services are created, delivered, and used.
Container technologies embody the modern, agile software development methodologies and enable operators to manage the full life-cycle of complex, application landscapes.

In this guide, you learned how to containerize an application, deploy that container, and access the application.
