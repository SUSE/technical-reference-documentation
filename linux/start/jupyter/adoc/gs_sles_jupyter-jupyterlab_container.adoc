...
The `podman build` or `buildah bud` command can build a container image from a sequence of instructions provided in a https://github.com/containers/common/blob/main/docs/Containerfile.5.md[Containerfile].
...
# add the unprivileged user's ~/.local/bin to PATH
ENV PATH="$JUPYTER/.local/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
...
CMD jupyter lab --ip=* --port=8888 --no-browser --notebook-dir=$JUPYTER/notebooks
----
...
For this guide, you use the 'latest' SUSE Linux Enterprise Python container image, which, at the time of this writing, is built on {productname}.
...
. Build the container image with podman or buildah.
+
[source, bash]
----
...
[source, bash]
----
buildah bud --layers --tag jupyter .
----
+
[TIP]
====
...
registry.suse.com/bci/python  latest      35ebb6a7981e  4 days ago      125 MB
----
...
podman run --name jupyter \
 -p 8888:8888 \
 -v $(pwd):/home/jupyter/notebooks:Z \
 localhost/jupyter
----
+
====
