...
For this script to do its job, you need to run it inside the container with the proper UID passed to it.
This is accomplished with the `ENTRYPOINT` instruction in your _Containerfile_ and the `--env` option of the `podman run` command.


=== Create Containerfile

The `podman build` can build a container image from a set of instructions provided in a file, called _Containerfile_.
If you are familiar with Docker, a _Containerfile_ shares the same format and most of the same options as a  _Dockerfile_.
