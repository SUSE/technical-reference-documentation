...
----
+
Replace `<release_version>` with the {product1_full} release you are using.
//
+
For example: If you are using {product1_full} {product1_version}, then you would replace `<release_version>` with `{product1_ver}`.
...
[NOTE]
====
The 'btrfs' driver is the default for {product1}, as https://en.wikipedia.org/wiki/Btrfs[Btrfs] is the default file system for system volumes.
...
You can set this without administrator help by creating or editing the file `$HOME/.config/containers/storage.conf` with the following contents:
...
One way to do this is by installing software RPM packages from the {product1_full} repositories.
This requires your user account to have read access to your SUSE Customer Center (SCC) credentials.

For this guide, you use the latest {product2_full} Python container image as your base.
...
All other required software and libraries can be installed from the {pypi_url}[{pypi_full}].
...
This does not impact your ability to build containers unless you need to install official {product1_full} RPM packages.
====

To install official RPM packages from the {product1_full} repositories, you need to grant read permission for your current user with:
...
In this example, you list only the names of the packages you want to install.
The `pip` dependency resolver attempts to install the latest, available and compatible versions of the specified packages.
This can result in unexpected versions (and behaviors) of the specified packages.

You can specify the exact version of each package you want to install.
For example, instead of `seaborn`, you could specify `seaborn==0.12.1`.
However, if the dependency resolver is unable to install the specified version of a package, the entire installation will fail.
...
Learn {containerguide_url}/cha-bci.html[why SUSE Linux Enterprise base container images] offer the ideal foundation for your application containers.
...
See the {podman_docs}/markdown/podman-run.1.html[Podman run documentation] for a more detailed discussion.
+
[NOTE]
====
With the release of Podman 4.3.0, user namespace mapping is simplified with the `--userns=keep-id` option.
...
You also learned how to use {product2_full} as a reliable, secure, supportable, and freely redistributable base layer for your containers.
