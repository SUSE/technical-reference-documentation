...
Setting the namespace mode to `keep-id` maps the UID and GID of the user inside the container to your UID and GID outside the container.
...
If you do run into ownership and permissions issues, you can try an older approach by using the `--uidmap` and `--gidmap` options by modifying the `podman run` command in your launch script as follows:
