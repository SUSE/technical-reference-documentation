...
By default, the user inside the container is not your regular user, but has a different UID and GID.
When you mount storage to the container, files will be accessed as the container user.
The problems resulting from file access permissions can introduce friction to your development workflows.

You can address this problem by running a script in the container that creates a user account with a matching UID and switches to this user to run your workload.

Create this script in your build directory and name it _entrypoint.sh_.

[listing]
----
#!/bin/bash

# DEBUG: set optional debug verbosity
[[ $DEBUG -eq 1 ]] && set -x

# specify container user information
#   using LOCAL_USERID passed with 'podman run'
...
For this script to do its job, you need to run it inside the container at the appropriate time and with the proper UID passed to it.
This is done with the `ENTRYPOINT` instruction in a _Containerfile_ and with the `--env` option of the `podman run` command.
