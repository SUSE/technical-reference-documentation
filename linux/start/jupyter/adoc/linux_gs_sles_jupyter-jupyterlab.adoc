...
Installing a different version of Python on your system is possible.
However, doing so incorrectly could break system tools, which have their own version dependencies.

To mitigate this risk, SUSE names the Python binaries explicitly.
For example, the default Python interpeter is the file, '/usr/bin/python3.6'.
It is symbolically linked as 'python3' to simplify usage and offer consistency.
So, when you issue the command `python3 --version`, this is actually executing, `python3.6 --version`.

Using this naming convention, it is actually possible to install multiple versions of Python.
Some additional versions may also be available in the standard operating system repositories.


[WARNING]
====
Be careful never to replace the system default Python or change any of the associated symbolic links, as this could break your system.
====


Maintaining and managing multiple versions of Python can be difficult.
This has led to the develppment of tools, like https://github.com/pyenv/pyenv[pyenv], which is out of scope for this guide.


For this guide, the default Python 3.6 is sufficient.
...
[NOTE]
====
Your Python virtual environment has its own `pip`, installed in the 'bin' subdirectory of your working directory (e.g., '$HOME/python3-ai/bin').
...
// TS (20220818): Brian, please flesh this out, incorporating the tip below.  Headless will be imprtant in the container world.
Then find the browser window that was opened by the preceeding command.

[TIP]
====
If you are running SLES on a headless server, you can use SSH port forwarding to access the service on port 8888.
====

. Explore the JupyterLab interface.
// TS (20220818): Brian, we may only need to do a screenshot and a link to the JupyterLab docs.
...
. Shut down the JupyterLab environment
// TS (20220818): Brian, for this, we should show how to properly shut down the environment.  This may require mentioning the difference between "Log out" and "Shutdown" in the interface.
...
== Creating repeatable installations

Replicabality is key to many disciplines, but particularly in the sciences.
Ensuring that your Python environment can be recreated the same way with the same packages and the same versions of those packages could be a challenge.

Fortunately, the `pip freeze` command can generate a package requirements file, containing a list of the packages and versions that should be installed.
...
----
+
Notice that there are a lot more files than the ones you explicitly installed.
This is because `pip freeze` captures all the packages and their versions in your environment.


Next, test that you can recreate the environment.

// TS (20220818): Brian, can you flesh this out?

. Deactivate your current virtual environment.

. Create a new virtual environment, named 'jup-test'.

. Activate the 'jup-test' environment.

. Update `pip`.

. Install packages from the 'python3-ai-req.txt' requirements file.
