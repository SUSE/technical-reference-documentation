...
{productname} comes with Python 3.6 pre-installed.
...
To avoid confusion and to avoid accidentally calling legacy Python 2.x, this guide uses explicit version commands, like `python3` and `pip3`.
====
...
----
+
The `deactivate` script resets all the environment changes (including your prompt).


[WARNING]
====
Before proceeding to the next section, reactivate the virtual environment with `source $HOME/python3-ai/bin/activate` or just `source ./bin/activate` from within the 'python3-ai' directory.
====
...
The Python `pip` command is the preferred installer for Python packages from the https://pypi.org/[Python Package Index (PyPI)].
