...
* Python3
+
// NOTE:  Python 3.6 is the default in SLES 15 sp4.  See:  https://scc.suse.com/packages/22551627
Python 3.6 is included with SLES 15 SP4 and is used in this guide.
...
SLES 15 SP4 comes with Python 3.6 pre-installed.
...
To run this example, we will need to install Kaggle API tools so we can download the example data.
We also need to install the Python dependencies for the regression code.
The instructions for that are below:
...
.Regression example - installation of Python dependencies
[source,bash]
----
$ pip install pandas
$ pip install seaborn
...
Now we are ready to try our regression example.
