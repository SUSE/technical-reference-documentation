...
:title: Replicable Python Environments with JupyterLab and SUSE Linux Enterprise: Getting Started
...
The https://www.python.org/[Python] programming ecosystem is popular among many developers for its simplicity, extensibility, cross-platform availability, and its large open source community of active developers and users.
Developers in many domains benefit from and contribute the large and growing Python ecosystem of open source tools and libraries.
This has resulted in a rapid state of continuous development for the language as well as for the domains where Python is used.
...
This guide helps you take a first step to setting up replicable Python development environments with https://www.suse.com/products/server/[SUSE Linux Enterprise], an operating system that is adaptable to any environment - optimized for performance, security, and reliability.
...
JupyterLab provides a great environment for interactive experimentation, and the notebook format provides an easy mechanism for sharing your code with others.
...
Whether you are just getting started on your Python development journey or already looking to scale to production, start from a capable, adaptable, and supported foundation with SUSE Linux Enterprise.
