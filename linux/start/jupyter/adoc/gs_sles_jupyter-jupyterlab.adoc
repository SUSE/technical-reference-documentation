...
In fact, you can have multiple versions of Python on your system.
But care must be taken not to break dependencies for system tools.

To mitigate this risk, SUSE uses explicit naming.
For example, the default Python interpreter is the file, `/usr/bin/{defaultpythonbin}`.
To allow consistency between minor version changes, this executable is symbolically linked as `/usr/bin/python3`.

So, you end up calling the default Python with the command:
...
Included in the standard {productname} repositories is a newer version of Python.
At the time of this writing, it is {newpython}.

Install {newpython} with:
...
[TIP]
====
You can list available Python 3 interpreters with `sudo zypper search -d "Python 3 Interpreter"` or `sudo zypper search python3`.
====

Once installed, you can use can use {newpython} by explicitly calling the correct executable.
...
It is recommended that you use the Python packages in the official SUSE repositories when possible, as these have undergone extensive testing and are supported by SUSE.
If you need another version of Python, you can build and install from source code or install an implementation from a trusted provider (such as https://www.anaconda.com/[Anaconda]).

[TIP]
====
Tools like https://github.com/pyenv/pyenv[pyenv] are available to help simplify installation and management of multiple Python environments, but this is out of scope for this document.
====
...
----
+
This creates a new 'myproj' subdirectory in your home directory and populates it with files, subdirectories, and symbolic links to enable the virtual environment to use the Python interpreter you specified in this command.
...
[NOTE]
====
Your Python virtual environment has its own *pip* executable and symbolic links to the Python interpreter you used to create it.
These are located in the 'bin' subdirectory of your working directory.
Use `python --version` and `pip --version` to verify.
====
