...
And scaling ML from a research project to enterprise production is no easy task, as IT and operations teams must securely manage implementation across a myriad of modern infrastructure options.

This guide helps you take a first step to setting up a modern ML development environment with https://www.suse.com/products/server/[SUSE Linux Enterprise], an operating system that is adaptable to any environment - optimized for performance, security, and reliability - on-premises, in the cloud, and at the edge.

The focus of this guide is on the https://www.python.org/[Python] programming language ecosystem, which has become very popular among ML researchers and developers, and interactive, computational notebooks with JupyterLab by https://jupyter.org/[Project Jupyter].
...
In either case, you need to explictly call the version of Python.
...
But Python is popular among ML developers for its simplicity, extensibility, cross-platform availability, and its large open source community of active developers and users.
ML developers benefit from and contribute to the large and growing Python ecosystem of open source tools and libraries.
...
In this guide, you will use pip to install packages from PyPI into your active virtual environment.


[NOTE]
====
Your Python virtual environment has its own 'pip' executable, installed in the 'bin' subdirectory of your working directory (e.g., '$HOME/myproj/bin').
...
Once you have a requirements file, you can share it with another developer or use it yourself to make sure a new environment contains all the same pacakges.
...
[source, bash]
----
pip install -r ../myproj/myproj-req.txt
----
...
+
//
The latest generation of Jupyter Notebook is a modular design that allows users to customize the interface to support workflows in data science, scientific computing, computational journalism, and machine learning.

* JupyterHub
+
//
The multi-user version of Jupyter Notebook enables companies, classrooms, and research labs to deliver computational environments and resources to users while managing them centrally.
...
A variety of community maintained kernels are available to support other programming languages, such as https://irkernel.github.io/[R], https://github.com/JuliaLang/IJulia.jl[Julia], and https://github.com/jupyter/jupyter/wiki/Jupyter-kernels[many others].
====
...
JupyterLab is a web application, so you will need a web browser to access it.
...
If this fails, you can manually open your browser to one of the URLs helpfully provided in your terminal window.
//
+
[NOTE]
====
...
//
+
Shutting down JupyterLab means stopping running extensions, kernels (which are separate processes for running code), terminals, and the UI.
Shut down JupyterLab directly from the UI by selecting __File > Shutdown__.
+
[NOTE]
====
...
The familiar menu bar provides access to a variety of functions, like loading and saving files, using edit functions, modify the interface view and other settings, and access help.
The menu also includes functions specific to working in an interactive computing environment, such as running code and managing compute kernels.

The sidebar offers a file browser and some additional controls and features.

JupyterLab opens with the main work area filled with the __Launcher__ tab.
From the Launcher that you can start a notebook, console, terminal window, and more.
...
[TIP]
====
You can enter multiple lines by just pressing ENTER at the end of each line.
...
. Save your notebook by selecting __File__, then __Save Notebook As...__.
//
+
Enter a name when prompted, such as 'mypythonversion.ipynb'.

. At this point, you can close your notebook by clicking the 'X' in the notebook tab.

. Reopen your notebook by finding it in the sidebar file browser and double-clicking it.


You can find more details about the JupyterLab UI in the https://jupyterlab.readthedocs.io/en/stable/user/interface.html#[documentation].
...
//=== Explore JupyterLab with Kaggle
//
//Next you will explore an example use case to show how you can use JupyterLab in your machine-learning project.
...
//
//Remember, you have explored this example notebook by using the
