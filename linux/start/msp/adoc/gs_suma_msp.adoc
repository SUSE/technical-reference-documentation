...
:subtitle: How MSPs Streamline Systems Lifecycle Management
...
:usecase: {product1} enables managed services providers streamline asset and lifecycle management across multi-client landscapes.

:executive_summary: {product1} enables managed services providers (MSPs) to streamline IT systems management with a single tool that provides systems asset management, systems provisioning, and automated software management to keep systems up-to-date and secure.
...
:author3_firstname: first (given) name
:author3_surname: surname
:author3_jobtitle: job title
:author3_orgname: organization affiliation
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
With {product1}, you can unify systems lifecycle management, streamline operations, and manage your growing IT landscape efficiently and effectively.


=== Scope

You learn how to deploy {product1} to support unified systems lifecycle management for multiple, downstream customers.
The hub and peripheral architecture described here enables managed services providers and large enterprises to consolidate management operations while maintaining the advantages of independent servers for each customer.
...
For this guide, you need two systems.
These can be bare metal or virtual machines.
One will act as a hub server and the other will act as a peripheral server.

As these servers will have very few clients, they do not need to follow normal sizing recommendations.
...
This server follows a standard installation and setup processes with the exception of enabling the Hub XMLRPC API service service.
...
. Install {product1} Server on the designated hub system.
//
+
See the https://{product1_docs}/installation-and-upgrade/installation-and-upgrade-overview.html[{product1} Installation and Upgrade Guide].
...
Creating a peripheral server follows the normal {product1} installation process.
This includes setting up the server database and administrator account.
However, instead of configuring the SCC connection, you configure the peripheral server to obtain SCC content from the hub.


. Install {product1} Server on the peripheral server.
//
+
See the https://{product1_docs}/installation-and-upgrade/installation-and-upgrade-overview.html[{product1} Installation and Upgrade Guide].

. Set up the database and certificates on the peripheral server.

.. At the command line, initiate the setup wizard:
+
[source, console]
----
...
.. Enter an email address to receive status notifications and click __Next__ to continue.
//
+
{product1} can sometimes send a large volume of notification emails.
You can disable email notifications in the Web UI after setup, if you need to.

.. Enter your certificate information and a password, then click __Next__.
//
+
[IMPORTANT]
====
...
.. From __SUSE Manager Setup__ > __Database Settings__, enter a database user and password, then click __Next__.

.. Click __Next__ to continue.

.. Click __Yes__ to run the setup and wait for it to complete.

.. Click __Next__ to continue.
+
[TIP]
====
Make note of the URL to the {product1} Web UI.
====

.. Click __Finish__ to complete {product1} setup.


. Create organization and the {product1} Administrator account.
//
+
[IMPORTANT]
====
The organization name and administrator account should match those on the hub server.
====

.. In your browser, enter the URL to the {product1} Web UI noted during setup.

.. Select __Create Organization__.

.. Fill in the fields.
+
image::suma_msp_create-org.png[Setup: Create Organization, scaledwidth="85%"]

. Click __Create Organization__.
        
After creating the organization, you are presented with the __SUSE Manager Home__ > __Overview page__.
In a single {product1} server architecture, you would normally proceed to enter SCC credentials and synchronize the SUSE Linux Enterprise product channels.
Instead, you next register this server as a peripheral of the hub.



=== Register a peripheral server
