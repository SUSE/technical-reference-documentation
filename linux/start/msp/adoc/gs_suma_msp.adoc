...
SUSE Manager is an enterprise, open source, infrastructure management solution for software-defined infrastructure.
SUSE Manager provides systems asset management, systems provisioning, and automated software management to keep your systems up-to-date.
It is designed to help enterprises reduce complexity and regain control of IT assets with a single tool to seamlessly manage Linux systems across a variety of hardware architectures and hypervisors as well as container, IoT, and cloud platforms.
With SUSE Manager you can effectively manage system lifecycle requirements across your IT landscape.


=== Motivation

With ever growing Linux footprints you need your management tool to scale to tens of thousands of Linux devices and beyond.
With the performance and scalability enhancements in SUSE Manager 4.3, you can easily scale in your environment in any direction, while providing better performance than any previous version even in very large-scale environments.
...
Organizations manage user access and permissions within a single SUSE Manager server.
...
You create one organization for each end customer, enabling you to keep the various managed systems, users, and software channels separate.


=== Content lifecycle management

Content lifecycle management involves creating clones of software channels at particualr point in time and assigning these to dofferent systems.
This is typically used to create multi-stage lifecycles where patches are passed from vendor channels to a development environment, from there to a test stage and then finally to production.
In our case, we will create separate lifecycles for each customer.
In this way, we can ensure that each customer receives only the patches that are appropriate to their respective software lifecycle needs even though they might all be based on the same upstream channels.
...
image::typical-hub-architecture-overview.svg[Hub Architecture, scaledwidth="85%", align="center"]
