...
You learn how to deploy {product1} to support systems lifecycle management for multiple, downstream customers.
...
// Mark: Don't we invalidate this by using the same organization (the MSP's organization) on all servers?

Organizations are basic building blocks of the solution.
...
image::suma_msp_hub-architecture.svg[Hub and peripheral topology, scaledwidth="85%", align="center"]
...
[NOTE]
====
For each peripheral server, you will need to create: activation keys, bootstrap, repositories, system groups, users, and so on.
...
Ideally, this could be through a shared (NFS) filesystem, but you can also copy the data across the network (such as with `rsync`).
...
Onboarding a new customer begins with adding the customer's SUSE Customer Center (SCC) credentials to the hub server.
SUSE Manager merges products coming from all SCC credentials.
...
The peripheral server can now be managed as a standard Salt client of the hub. 
...
* https://{product1_docs}/client-configuration/registration-webui.html[with the Web UI]
...
[IMPORTANT]
====
For all methods, the client must have the date and time synchronized with the {product1} server.
====
...
// Mark: After a general statement link to external documentation for the Centralized management section, I don't think we need to go over Salt modules and formulas. I'm going to go ahead and comment out this subsection.
...
// Mark: Do we need any of this Hub API section?
...
// Mark: If we are not going to walk through an example, I suggest we delete the content commented out below:
