...
To follow the steps outlined in this guide for yourself, you need at least two systems.
...
You also need a way to transfer data between the hub and peripheral.
...
However, for the hub server, you must also enable the Hub XMLRPC API service service.
...
To onbooard a new customer, you start by adding the customer's SUSE Customer Center (SCC) credentials to the hub server.
...
Once the SCC credentials have been added, you have a centralized view of all products that are available through the hub and the ability to replicate entitlements from the hub to peripheral servers.
...
[NOTE]
====
You may want to create an activation key specifically for peripheral servers, especially if you will have other types of clients registered to your hub.
====
...
When the bootstrap process has completed, your client is listed in the WebUI at __Systems__ > __System List__.
The peripheral server can now be managed as a standard Salt client of the hub and patched in the normal way.


== Synchronizing content

You need to ensure that your {product1} servers stay aligned on content and permissions.
This is accomplished with the help of Inter-Server Synchronization.
...
These are then assigning to appropriate, managed clients.

For example, suppose you have a customer called ACME Corp.
You can create a Content Lifecycle Project just for this customer as follows:
...
This directory and its contents needs to be transferred to to the peripheral server.
The easiest way to do this is to use a shared filesystem.

The example below shows how to export the development environment from the ACME Corp customer's CLM to an NFS filesystem mounted at `/mnt/shared` on the hub and peripheral servers.
...
[NOTE]
====
There is no need to specify channel names because that is impicit in the SQL statements.
...
[IMPORTANT]
====
Since activation keys make reference to channels, so you need to import appropriate content first.
====
...
Install this package on the {product1} hub server by logging in as root and issuing the commnad:
...
Earlier, you installed the `hub-xmlrpc-api` package and enabled and started the service.
...
With the service started, you can connect to it at port 2830 using any XMLRPC-compliant client libraries.
...
{product1} provides some pre-configured reports that you can leverage for a variety of tasks, such as taking inventory of subscribed systems, users, and organizations.
