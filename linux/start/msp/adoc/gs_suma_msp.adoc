...
https://documentation.suse.com/suma/4.3/en/suse-manager/specialized-guides/large-deployments/multi-server.html[{product1} Hub] was introduced to manage very large deployments by allowing horizontal scaling to multiple servers.
In this configuration, one {product1} server acts as the central (or *Hub*) server.
Then one or more additional {product1} servers (or *peripherals*) are registered to the Hub as Salt clients.
Any number of clients (proxies and endpoints) can be registered to the peripheral servers.

A typical {product1} Hub and peripheral topology is illustrated here:

image::typical-hub-architecture-overview.svg[Hub Architecture, scaledwidth="85%", align="center"]


It is important to note that each peripheral server is an independent {product1} server.

Immediately,  see the value of this notion in the MSP environment – each customer could have a dedicated SUSE Manager server. This would address concerns about separation of customers, while the hub will still allow centralized control for the Service Provider.
