...
=== Enable Inter-Server Synchronization (ISS)

Inter-Server Synchronization (ISS) helps you keep your {product1} peripheral servers up-to-date with your hub.
That is, it helps you ensure that content and permissions stay aligned.

The `inter-server-sync` package must be installed on both source (hub) and target (peripheral) servers.
...
=== Build Customer-Specific CLM Channels

With {product1} you can create customer-specific channels for robust Content Lifecycle Management.

// More here 


=== Export and Import CLM Channels

// This needs to be fleshed out and formatted

Use the `inter-server-sync` command to export the CLM channels from the hub to a local filesystem directory.
For example:
