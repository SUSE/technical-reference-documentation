...
Managing a large and growing IT footprint can be challenge for any enterprise.
...
An MSP with multiple large customers could reach this limit fairly easily.
To accommodate this, {product1} can be scaled horizontally by adding {product1} servers.

Separation of entitlements::
{product1} uses customer credentials to access software entitlements from upstream providers like, for example, the SUSE Customer Center (SCC).
It would not be a good idea to mix the entitlements of multiple customers in a single environment – it would be difficult to keep track of which managed systems are entitled to which products.
