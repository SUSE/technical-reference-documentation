...
Follow https://{product1_docs}/installation-and-upgrade/server-setup.html#_synchronizing_products_from_suse_customer_center[Synchonizing Products from SUSE Customer Center] to onboard a customer.
...
{product1} is compatible with a wide range of end-point (or client) technologies.
You can install traditional or Salt clients, running SUSE Linux Enterprise or another Linux operating system, with a range of hardware options.
For a complete list of supported clients and features, see https://{product1_docs}/client-configuration/supported-features.html[Supported Clients and Features].

Registering end-points to a peripheral server is no different to registering end-points to any {product1} server.
See https://{product1}/client-configuration/registration-overview.html[Client Registration] for specific steps and guidance.
...
Managemnt of users, system groups, activation keys, and so on.
...
// Summarize what was covered and provide some next step suggestionds.
