...
deployment, a viable approach is to deploy as a virtual machine (VM) on the target nodes, on top of an existing hypervisor, like KVM. For a physical host, there a re tools that can be used during the setup of the server, see below.
...
*** Utilize the Discover function from {vn_HPE} {vn_HPE_ComposerTech} to see all of the available nodes that can be assigned to to their respective roles:
...
. Then for the solution VM, utilize the hypervisor user interface to allocate the necessary CPU, memory, disk and networking as noted in the link:{pn_Rancher} {pn_Rancher_HWReqURL}[hardware requirements].
