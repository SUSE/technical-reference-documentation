...
* Rack server deployment flexibility - {an_Cisco} {an_CiscoUCS} {familyCiscoUCS-C} unique in the industry because they can be integrated with {an_Cisco} {an_CiscoUCS} connectivity and management or used as stand-alone servers
* {pn_Cisco_BMC} ({an_Cisco_BMC}) - Running in the system’s Baseboard Management Controller (BMC), when a {an_Cisco} {an_CiscoUCS} {familyCiscoUCS-C} is integrated into a {an_Cisco} {an_CiscoUCS} domain, the fabric interconnects interface with the {an_Cisco_BMC} to make the server part of a single unified management domain. When a server is used as a standalone server, direct access to the {an_Cisco_BMC} through the servers’s management port allows a range of software tools (including {an_Cisco} {pn_Cisco_Intersight}) to configure the server through its API.
