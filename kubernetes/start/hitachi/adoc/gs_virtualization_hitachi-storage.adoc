...
+
Log in to all three nodes of {svirt} and run the following commands to start and enable the multipathd service on all three nodes.
+
[source, console]
----
...
.. create multipath.conf file at path /etc/multipath.conf (adjust vendor/product rules to match your array):
+
[source, yaml]
----
   defaults {
...
----
.. Once file /etc/multipath.conf created, restart multipathd service.
+
[source, console]
----
...
. Install {hspc-long}. do all below steps on any one of the master node of {svirt} cluster
...
+
.. Customize hspc_v1_hspc_vspone.yaml with your namespace and settings, then apply with Kubectl.
+
[source, yaml]
----
...
.. Encode credentials to base64 and create secret using secret-vsp-112.yaml .
+
[source, yaml]
----
   apiVersion: v1
...
----
.. use Kubectl command to create secret 
...
.. use Kubectl command to create StorageClass
...
----
    ...
    kubectl apply -f hitachi-snap.yaml
...
image::hitachi-disks.png[Architecture diagram, scaledwidth="85%", align="center"]

Power on the VM and verify the guest OS sees the attached disk and that I/O works.

image::hitachi-vm-running.png[Architecture diagram, scaledwidth="85%", align="center"]
