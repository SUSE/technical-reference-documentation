...
* You have access to an OCI image registry that is available to the build host AND the target Kubernetes cluster. This will allow the GPU driver to be deployed across all Kubernetes worker nodes that have Nvidia GPUs, when installing the Nvidia Kubernetes Operator Helm chart.
//
+
[NOTE]
====
