...
:product-sle: SLE
:product-sle-full: SUSE Linux Enterprise 
...
* Build an OCI compliant container image that incorporate an appropriate {nvidia-drv} within the 
is dedicated to helping you create  SUSE Linux Enterprise 15 Base Container Image. Additionally, it provides instructions for deploying these container images within a Kubernetes cluster, specifically {product-rke2}.
...
* You have access to a SUSE Linux Enterprise 15 build host. You should ensure the build host is the same Service Pack (e.g. {product-bci-version}) version as the {product-bci} image to be used. NOTE: The build host does not require access to an Nvidia GPU.
...
* You have installed the required {product-sle-full} Nvidia software packages on each Kubernetes worker node that is configured with an NVIDIA GPU. You can use the following commands to install the appropriate software.
//
+
[console, bash]
----
...
* You have access to a Kubernetes cluster that is equipped and correctly configured with Data Center class Nvidia GPUs. These instructions leverage SUSE's security focused Kubernetes distribution, {product-rke2}. 
...
## Provide the {nvidia-drv} version (e.g. {gpu-driver-version})
## Find the latest "Data Center Driver for Linux x64" version for your GPU at {nvidia-driver-download-url}
export DRIVER_VERSION=""
## Provide the {nvidia-op} version (e.g.{gpu-operator-version})
## Find the associated {nvidia-op} version at {nvidia-driver-gpu-operator-url}
export OPERATOR_VERSION=""
## Provide the required CUDA version (e.g. {cuda-version}) for {nvidia-drv} by selecting your driver version at {nvidia-driver-gpu-operator-url}
...
.Step 4: Build the {product-sle} container image with the {nvidia-drv} installed:
...
NOTE: This step is optional and requires running the newly created {nvidia-drv} container locally with `Podman`, outside of the Kubernetes context. This can be done on a {product-bci-version} host configured with the same kind of Nvidia GPU the container was created for, or on a Kuberentes worker node that is configured with an Nvidia GPU.
...
A pivotal point to underscore is the indispensability of a Kubernetes cluster, preferably {product-rke2}, that is provisioned with Nvidia GPU support and the requisite Nvidia drivers to fully harness GPU resources when deploying containers integrated with the aforementioned OCI compliant container images.

For an exhaustive comprehension of advanced configurations and nuanced details, the consultative resources made available by SUSE, Docker, Nvidia, and {product-rke2}are strongly recommended.
