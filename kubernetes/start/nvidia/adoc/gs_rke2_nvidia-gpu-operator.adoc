...
sudo SUSEConnect -p sle-module-{nvidia-name}-compute/15/x86_64
----
...
sudo zypper install \
  ...
  sle-module-{nvidia-name}-compute-release
----
...
The process of building the OCI compliant {nvidia-drv} image using {product-bci} and deploying it to an {product-rke2} cluster will leverage the following primary components:
//
+
...
The process of building the OCI compliant and deploying it to an {product-rke2} cluster will entail the following steps:
//
+
...
//
+
.. Open a command line session to the build host or Kubernetes worker node on which you will test the container image.
//
+
...
This guide has guided you in the creation of OCI compliant container images leveraging the {product-bci-full} and incorporating the {nvidia-drv}. 
As well, it has provided coherent instructions for validating the functionality of the container image and the seamless deployment of the image within a Kubernetes cluster, specifically {product-rke2}. 

This integrated solution is designed for containerized applications GPU-acceleration, while centralizing the deployment of the {nvidia-op} software across any size Kubernetes cluster.
