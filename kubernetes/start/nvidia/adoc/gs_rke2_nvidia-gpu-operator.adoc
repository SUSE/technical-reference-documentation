...
Before embarking on the procedures outlined in this guide, it is imperative to ensure the fulfillment of the following prerequisites:
   ...
   1. You have access to a Kubernetes cluster that includes at least one node with a Data Center class Nvidia GPU. These instructions will focus on SUSE's security focused Kubernetes distribution, RKE2. 
...
Step 7: (Optional) If the specified build registry was not set to "local", push the newly build image to the container registry:
