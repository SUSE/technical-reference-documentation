...
It is assumed that the Nvidia GPU(s) being used are Data Center class. It is beyond the scope of this document to integrate Consumer GPUs.
...
This section offers a detailed explanation of the steps required to create OCI compliant container images with the Nvidia GPU Operator on the SUSE Linux Enterprise 15 SP5 Base Container Image.
...
Step 4: Build the SLE15 container image with the Nvidia GPU operator installed:
