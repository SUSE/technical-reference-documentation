...
The first is the creation of an OCI compliant container image based on the SUSE Linux Enterprise 15 Base Container Image, that runs the {nvidia-drv}. 
...
The choice of SUSE's SLE 15 Base Container Image is motivated by the unparalleled security certifications and enhanced supportability it offers, particularly when operating heterogeneous software stacks. 
SUSE's SLE 15 Base Container Image is distinguished by a wide range of security certifications, including Common Criteria, FIPS, and EAL, making it a trusted choice for organizations with stringent security requirements.
...
It assumes a foundational understanding of Podman and/or Docker, Kubernetes, and Nvidia GPU technologies. 
...
* You have access to a SUSE Linux Enterprise 15 build host. 
...
* You have installed the required {product-sle-full} Nvidia software packages on each Kubernetes worker node that is configured with an NVIDIA GPU. 
...
* Podman is installed on the build host and at least one of the Nvidia GPU equiped Kubernetes worker nodes. 
...
This will allow the {nvidia-drv} to be deployed across all Kubernetes worker nodes that have Nvidia GPUs, when installing the {nvidia-op} Helm chart.
//
+
[NOTE]
====
...
* You have access to a Kubernetes cluster that is equipped and correctly configured with Data Center class Nvidia GPUs. 
...
[TIP]
====
Disconnecting from your current terminal envionment will cause the variables to be lost. 
...
If so, select the image located in docker.io.
====
//
+
[NOTE] 
...
[IMPORTANT] 
====
If the target container registry requires authentication, use the `sudo podman login` command to successfully authenticate before continuing. 
...
This can be done on a {product-bci-version} host configured with the same kind of Nvidia GPU the container was created for, or on a Kuberentes worker node that is configured with an Nvidia GPU.
====
...
A pivotal point to underscore is the indispensability of a Kubernetes cluster, preferably {product-rke2}, that provides full Nvidia GPU support and the requisite {nvidia-op} to fully harness GPU resources when deploying GPU intensive workloads.
