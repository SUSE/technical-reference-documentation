...
// -
:product-bci: SLE BCI
:product-bci-full: SUSE Linux Enterprise Base Container Images
:product-bci-version: 15 SP5
:product-bci-registry: https://registry.suse.com
:product-bci-url: {product-bci-registry}/bci/bci-base-15sp5/index.html
:product-rke2: RKE2
:product-rke2-full: Rancher Kubernetes Engine 2
:product-rke2-url: https://www.suse.com/products/rancher-kubernetes-engine/
:product-rke2-support-matrix: https://www.suse.com/suse-rke2/support-matrix/all-supported-versions/
:product-rke2-version: 1.25
:product-sle: SLES
:product-sle-version: 15
:product-sle-sp-version: SP5
:product-sle-sp-version-short: 5
:product-sle-version-full: 15 SP5
...
:sle-docs: https://documentation.suse.com/sles/{product-sle-version}-{product-sle-sp-version}
:sle-docs-modules: {sle-docs}/single-html/SLES-modules/#sec-modules-install
...
:executive_summary: This guide describes a process for building {nvidia-drv} enabled container images with {product-bci-full} and deploying these containers with the {nvidia-op} on {product-rke2-full}.
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
. create OCI-compliant container images with {product-bci-full} that contain the {nvidia-drv}

. deploy a GPU-enabled container image on {product-rke2-full} with the {nvidia-op}

{product-bci-full} ({product-bci}) are a trusted choice for organizations with stringent security requirements, providing unparalleled security certifications (including Common Criteria, FIPS, and EAL) and enhanced supportability even when operating heterogeneous software stacks.
...
* building an OCI-compliant container image on {product-bci-full} that includes the {nvidia-drv}

* validating the GPU-enabled container image

* publishing the image to a container image registry

* deploying the container image with the {nvidia-op} Helm chart to a {product-rke2} cluster
...
[IMPORTANT]
====
This guide assumes that you are using Data Center class {nvidia-name} GPUs. Integrating consumer grade GPUs is outside the scope of this document.
====
...
To be successful, you need a foundational understanding of Podman or Docker, Kubernetes, and {nvidia-name} GPU technologies.
...
====
+
Ensure your build host has:
+
--
* a modern Linux operating system
//
+
This guide uses {product-sle-url}[{product-sle-full}] {product-sle-version-full}.
It is strongly recommended that your build host run the same operating system as the {product-bci-full} image you will use (matching version and service pack).

* the following {sle-docs-modules}[modules enabled]:
** Containers
** Desktop Applications
** Development Tools

* access to {product-sle-full}
//
+
Ensure that your build host can access and download base container images from the {product-bci-registry}[SUSE container registry].
--
...
+
--
Your Kubernetes cluster must haver worker nodes that are equipped and configured with Data Center class {nvidia-name} GPUs.
You will leverage the security focused Kubernetes distribution, {product-rke2-full}, developed by SUSE.

Be sure that the Containers Module and {nvidia-name} Compute Module are enabled on the worker nodes of your Kubernetes cluster.
+
[NOTE]
====
...
* Podman is installed on the build host and at least one of the {nvidia-name} GPU equipped Kubernetes worker nodes. 
...
* An {product-rke2} Kubernetes cluster with at least one worker node that is configured with an {nvidia-name} GPU. 
* Access to the {product-sle-full} software modules through the {scc-full} or an {product-rmt} server.
...
//
+
. Update the Dockerfile's base container image to the {product-sle} {product-bci-version} BCI:
//
+
[source, console]
----
...
This can be done on a {product-bci-version} host configured with the same kind of {nvidia-name} GPU the container was created for, or on a Kubernetes worker node that is configured with an {nvidia-name} GPU.
====
//
+
...
//
+
.. Before continuing to the Kubernetes deployment procedure, ensure the {nvidia-name} kernel modules are not loaded on any of the {nvidia-name} GPU equipped Kubernetes worker nodes:
//
+
[source, console]
----
