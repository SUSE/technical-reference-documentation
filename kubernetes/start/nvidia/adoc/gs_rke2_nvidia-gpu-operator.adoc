...
Step 1: Set these variables that will be consumed throughout the process:
...
Step 4: Find the latest "Data Center Driver for Linux x64" version for your GPU at https://www.nvidia.com/en-us/drivers/unix/, or https://www.nvidia.com/download/index.aspx
...
NOTE: If any of the variables are not set correctly, press `Ctrl+c` and return to Step 1 in this process before continuing
...
sudo podman push ${REGISTRY}/nvidia-sle15sp${SLE15_SP_VERSION}-${DRIVER_VERSION}
----

* Verify the image is saved in the registry, and remotely available:

----
sudo podman search ${REGISTRY}/
----

Step 9: Validate the container image

NOTE: This step is optional and requires running the GPU operator container locally with `Podman`, outside of the Kubernetes context. This can be done one a SLE15 node configured with an Nvidia GPU, or a Kuberentes worker node that is configured with an Nvidia GPU
...
* Create the `/run/nvidia` directory, in case it has not yet been created:
...
* Watch the deployment of the Nvidia GPU operator:
...
* Verify that the `nvidia-smi` utility can communicate withe the GPU:
...
helm install --wait --generate-name \
  ...
  --set driver.image=nvidia-sle15sp${SLE15_SP_VERSION}-${DRIVER_VERSION} \
