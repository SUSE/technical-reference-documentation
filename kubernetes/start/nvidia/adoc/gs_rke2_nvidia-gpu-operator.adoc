...
* The {product-sle} Containers Module has been enabled on the build host and the {product-sle} Containers Module plus the NVIDIA Compute Module have been enabled on all Kubernetes worker nodes. 
...
* Create the {build-variables-file} file:

[console, subs="attributes"]
----
cat <<EOF> {build-variables-file}
## Provide the URL of the registry where the new image will be saved:
export REGISTRY=""
## Enter the number for the {product-sle} {product-sle-version} service pack to be used for the container image (i.e. {product-sle-sp-version-short} for {product-sle-sp-version})
...
* Edit the {build-variables-file} file to provide the appropriate values.
...
.Step 2: Clone the Nvidia GitLab repository and change to the `{nvidia-git-lab-directory}` directory:
//
+
[console, subs="attributes"]
----
...
In addition, if the container build host is a different system than the one being used to perform the Helm install, the {build-variables-file} file will need to be created on the second system. 
