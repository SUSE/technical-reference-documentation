...
This Getting Started guide provides comprehensive instructions for two imporant tasks. The first is the creation of an OCI compliant container image based on the SUSE Linux Enterprise 15 Base Container Image, that runs the Nvidia GPU driver. The second is deploying the container image on {product2_full}. The primary objective is to enable you to seamlessly integrate Nvidia's Kuberentes GPU Operator, simplifying GPU management and support within Kubernetes clusters for GPU-intensive workloads. 
...
## Find the associated GPU Operator version at {nvidia-driver-gpu-operator-url}
export OPERATOR_VERSION=""
## Provide the required CUDA version (e.g. 12.2.2) for GPU driver by selecting your driver version at {nvidia-driver-gpu-operator-url}
