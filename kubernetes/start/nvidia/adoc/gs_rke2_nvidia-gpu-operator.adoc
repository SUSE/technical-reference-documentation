...
Before embarking on the procedures outlined in this guide, you should ensure the fulfillment of the following prerequisites:

   1. You have access to a SUSE Linux Enterprise 15 build host. It is highly recommended to ensure the build host is the same Service Pack (e.g. SLE15 SP5) version as the SLE BCI image to be used. NOTE: The build host does not have to have access to an Nvidia GPU.
...
NOTE:  Find the latest "Data Center Driver for Linux x64" version for your GPU at https://www.nvidia.com/download/index.aspx. 
