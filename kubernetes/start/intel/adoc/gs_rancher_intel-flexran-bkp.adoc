...
// -
:title: Intel FlexRAN Reference Solution Setup for Suse
:subtitle: FlexRAN Deployment Guide for Rancher Kubernetes Cluster
:productname: SLES 15 SP3 RT, RANCHER 2.6, SLE Micro 5.2 RT
...
== Introduction
This document is providing detailed steps to setup a cloud-native stack for Intel FlexRAN on Suse/Rancher kubernetes cluster.
This demo demonstrate the steps required to install and configure Intel FlexRAN PHY Reference Design using SUSE Linux Enterprise Server Real Time as the base Operating System and SUSE Linux Enterprise Micro Real Time as target hosts for Rancher RKE2 or K3S kubernetes cluster with SUSE Rancher Server to manage this deployment. 
The {otherproduct1}™ is a 4G and 5G baseband PHY Reference design which uses Xeon series processors and consists of a L1 library for SkyLake/CascadeLake platforms to demonstrate the capabilities of the software running 5GNR L1 features. See  https://www.intel.com/content/www/us/en/developer/topic-technology/edge-5g/tools/flexran.html [FlexRAN™ Reference Architecture] for more information.

"needs some cool graph here to show architecture"...I copied one from Dell doc, just as an example
image::suse/rancher-architecture1.png[Suse and FlexRAN architecture, scalewidth="75%", align="center"]
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
I need something similar like Dell put in their doc....Some fancy marketing words like Suse/Rancher will help you to provide a low cost kubernetes solution, easy to deploy and manage at scale to simplify Telco solution, blah.. blah...
...
This guide will help you to deploy a functional solution for Telco with FlexRAN/Suse kubernetes cluster with simple tests.
...
The following hardware components were used during the development of this guide:
...
image::eth1.png[networking components, scalewidth="75%", align="center"]

[NOTE]
====
 For more details on required components check Intel FlexRan reference documentation: Installation Guide Software Release v22.07 (Doc. No.: 575834-15.0) and FlexRAN™ 5GNR Reference Solution 22.07 - PHY Software Documentation (Doc. No.:603577)
====

image::lscpu.png[CPU, scalewidth="75%", align="center"]
...
image::bios1.png[bios1,scalewidth="75%", align="center"]
...
image::bios2.png[bios2,scalewidth="75%", align="center"]

image::bios3.png[bios3,scalewidth="75%", align="center"]

image::bios4.png[bios4,scalewidth="75%", align="center"]

For more details, please review section 2.4.2 of FlexRAN Software Reference Solution Cloud-Native Setup. (Doc. No. 575834-15.0) and BIOS Settings for FlexRAN Platforms Based on Intel ® Xeon® Processors. (Doc. No.: 640685, Rev.: 1.3)

* OS prerequisites

FlexRan requires to run on the Real-Time platform, as listed in Intel FlexRan Installation Guide (Doc. No.: 575834-15.0)
For more details about SLES RT please review https://www.suse.com/products/realtime/
//
+
Install SUSE Linux Enterprise Server 15 Real Time. Please refer to https://documentation.suse.com/sle-rt/15-SP3/
//
When installing SLE 15 RT, during the installation make sure to unmark kernel-default.

image::slesRT-install1.png[sle RT installation, scalewidth="75%", align="center"]

Verify that kernel-rt is selected

image::slesRT-install2.png[sle RT installation 2, scalewidth="75%", align="center"]
...
Isolate cores
//
Verify that tuned installed.
[source, bash]
----
zypper in tuned*
----

image::tuned1.png[tuned1,scalewidth="75%", align="center"]

In our case we have 1 socket and 40 cores

Add isolated cores in the configuration
...
image::tuned2.png[tuned2,scalewidth="75%", align="center"]
//
+
Activate RT profile
//
...
----
//
image::grub1.png[grub1,scalewidth="75%", align="center"]
...
----
image::grub2.png[scalewidth="75%", align="center"]

[source, bash]
----
cat /proc/cmdline
----
image::grub3.png[scalewidth="75%", align="center"]
...
image::cpu-avx.png[scalewidth="75%", align="center"]

In this test Xeon 4316 was used with 2.6 GHz.

There are several options to setup your CPU frequency.
//
+
1. Use cpupower tool
//
For example.
By running [source, bash]
----
 cpupower frequency-info
----
you can check available frequencies for your CPU and drivers.

image::frequency1.png[scalewidth="75%", align="center"]
...
[NOTE]
====
It’s important to set C-state and P-state on the Bios settings as well as on the kernel side. If you won't do this, you won't be able to change governors from the cpupower command and set the cpu frequency. Also, make sure that the BIOS can be changed from the OS by proper setting.
====
...
image::turbostat1.png[scalewidth="75%", align="center"]

You can also check with other available tools

image::mperf.png[scalewidth="75%", align="center"]

image::cpuinfo.png[scalewidth="75%", align="center"]
//
+
2.You can also use a 2nd method to change AVX512 frequency and install Intel msr-tools with the following commands:
//
[source, bash]
----
...
image::avx2.png[scalewidth="75%", align="center"]

Run the above bash script with your specific numbers which should change to the required frequency and verify that required frequency was applied.

Review performance with a cyclictest [SLE RT Hardwawre Testing] https://documentation.suse.com/sle-rt/15-SP3/pdf/article-hardware-testing_color_en.pdf


image::cyclic1.png[scalewidth="75%", align="center"]
...
image::oneapi1.png[scalewidth="75%", align="center"]

image::oneapi2.png[scalewidth="75%", align="center"]
//
+
Source the env and verify installed version.
//
image::oneapi3.png[scalewidth="75%", align="center"]

Make sure that GCC is installed to work with ICX compiler.

image::gcc.png[scalewidth="75%", align="center"]
...
image::pkgconf.png[scalewidth="75%", align="center"]
...
image::source-flexran.png[scalewidth="75%", align="center"]
...
Review possible compilation options from FlexRan.

image::flexran-options.png[scalewidth="75%", align="center"]
...
image::dev-bind.png[scalewidth="75%", align="center"]
...
image::pf-bb1.png[scalewidth="75%", align="center"]
//
+
For VF option:
//
Create 2 VFs from the PF:
image::max-vf.png[scalewidth="75%", align="center"]
...
----
image::dev-bind2.png[scalewidth="75%", align="center"]
...
image::acc2.png[scalewidth="75%", align="center"]
...
----
image::bbdev-test.png[scalewidth="75%", align="center"]

== Test

=== TestMac sample test

Follow steps from the TestMac section of FlexRAN 5GNR Reference Solution 22.07 PHY Software Documentation - Document #603577

image::testmac1.png[scalewidth="75%", align="center"]

[NOTE]
====
Always source Flexran env and the oneAPI in each tab and make sure that all paths are exported.
...
----
For example:
image::timer1.png[scalewidth="75%", align="center"]
...
image::console1.png[scalewidth="75%", align="center"]
...
image::console2.png[scalewidth="75%", align="center"]

See examples from Document #603577 TestMac section.
image::testmac-2.png[scalewidth="75%", align="center"]

The connection should be established in the 1st terminal once you’ll run l2.sh from the 2nd terminal.
image::console2-2.png[scalewidth="75%", align="center"]

In the 2nd terminal you should be able to see test result.
image::console2-3.png[scalewidth="75%", align="center"]

Another example is to use preconfigured test file.
...
----
image::console2-4.png[scalewidth="75%", align="center"]
image::console2-5.png[scalewidth="75%", align="center"]
image::console2-6.png[scalewidth="75%", align="center"]

[NOTE}
====
Number of failed tests listed above related to number of cores defined in the test file.
====
...
Creating a cset called flexran_set

image::cset1.png[scalewidth="75%", align="center"]

Example of moving <top> command from root set to flexran_set 
image::cset2.png[scalewidth="75%", align="center"]

For example when starting a testmac you can move pid to a dedicated cset

image::cset3.png[scalewidth="75%", align="center"]
image::cset4.png[scalewidth="75%", align="center"]

 To move all siblings from pid use –threads   
[source, bash]
----
cset proc -m -p 16165 --threads -t two
----

For all features of cpu manipulations please review shielding tasks document for cpu isolations.
...
To run Testmac with VF set, change setting to proper VF value and config.
image::vf1.png[scalewidth="75%", align="center"]
...
image::testmac-3.png[scalewidth="75%", align="center"]

image::cset5.png[scalewidth="75%", align="center"]

image::cset6.png[scalewidth="75%", align="center"]
...
== Deploying FlexRan on Container through Kubernetes

=== Generating LTE/5G Docker Images with pre-build FlexRAN

All prereq and FlexRan should be installed
The main doc to follow -> FlexRAN Reference Solution Cloud-Native Setup (Document Number: 575834-15.0)
...
//
+
Modify flexran_build_dockerfile.sh from flexran directory
image::flexran-build-script.png[scalewidth="75%", align="center"] 

[NOTE]
====
Modify according to your local setup.
<ADD http://...>  is the IP of the local RMT server.
...
image::docker1.png[scalewidth="75%", align="center"]
...
image::docker2.png[scalewidth="75%", align="center"]
...
For that you need to replace ‘docker build’ command with podman in flexran_build_dockerfile.sh file with
podman build -t
For more details review the Podman guide > https://documentation.suse.com/sle-micro/5.1/pdf/article-podman_color_en.pdf
====

Prepare file to export to the target node
Save docker as
[source, bash]
----
...
=== Creating RKE2 cluster

* Install a Rancher server

Sle Micro 5.2 was used as a Rancher server host.
...
For more details on installation, review > https://documentation.suse.com/sle-micro/5.1/pdf/book-deployment-slemicro_color_en.pdf

You can enable a cockpit console after the installation for easy management.
 # systemctl enable --now cockpit.socket
and open console in the browser as https://your-ip:9090/

For exzmple

image::rancher1.png[scalewidth="75%", align="center"]

For more details review:
https://links.imagerelay.com/cdn/3404/ql/c90a9c7f472e4a908936e385cd330a5b/suse-linux-enterprise-micro-5.1-solution-sheet.pdf
...
----
image::pods.png[scalewidth="75%", align="center"]
...
For more details on installation, review > https://documentation.suse.com/trd/kubernetes/pdf/kubernetes_ri_rancher-k3s-slemicro_color_en.pdf

* Create a custom rke2 cluster

From the Rancher server create a custom cluster > switch to rke2

image::rancher2.png[scalewidth="75%", align="center"]

Copy registration script to a new node to add it to the cluster

image::rancher3.png[scalewidth="75%", align="center"]

Verify if machines got provisioned.
image::rancher4.png[scalewidth="75%", align="center"]
...
In this case we have 2 nodes with Sle Micro 5.2 RT installed as part of RKE2 cluster.
Both target nodes should have DPDK with a patch and Intel oneAPI installed.
[NOTE]
====
...
For core isolation on Sle Micro RT, install tuned package with additional prereq. 
[source, bash]
----
...
[NOTE]
====
 Sle 15 repo was used for that with curl commands to download locally.
====

Modify /etc/default/grub to the required tuned parameters with isolcpu and run transactional-update grub.cfg to save changes and reboot. 
During our RKE2 cluster deployment, we selected Multus and Calico as our default plugins, so no needs to install them manually.
When setting up CPU Manager for Kubernetes* (CMK*) it should be based on isolcpu settings in GRUB

For the rest required plugins, follow section 4.
...
----
image::docker3.png[scalewidth="75%", align="center"]
...
image::pods3.png[scalewidth="75%", align="center"]
...
image::dev-bind3.png[scalewidth="75%", align="center"]
...
image::acc3.png[scalewidth="75%", align="center"]

image::acc4.png[scalewidth="75%", align="center"]
...
image::configmap1.png[scalewidth="75%", align="center"]
where intel_fec_5g is ACC100 accelerator card
...
[source, bash]
----
/var/flexran/build/docker # kubectl create -f flexran_testmac_mode.yaml
----

image::pods4.png[scalewidth="75%", align="center"]
...
Intel provided with a simple testmac script to verify functionality.
This test is similar to what was used on the physical SLES RT node.
...
As an ideal solution, pre-configured Intel FlexRan helm chart as well as all required cni plugins, can be posted on Rancher marketplace to simplify deployment at a scale with Rancher rke2 cluster.
====
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
...
This deployment guide demonstated a working example of how to preconfigure and create Intel FlexRAN container images and deploy it on a Rancher kubernetes cluster.
...
== Reference
    ...
    • https://infohub.delltechnologies.com/static/media/client/7phukh/DAM_d7aa83cb-f6bc-4b96-8baa-8ce78598cc9b.pdf

    • https://documentation.suse.com/sle-rt/15-SP3/

    • https://documentation.suse.com/sles/15-SP3/pdf/book-container_color_en.pdf

    • https://documentation.suse.com/sle-micro/5.1/
