...
You can install {productname} from the {market}, enabling you to manage all of your downstream clusters wherever a routable API address exists.
...
In this guide, you learn how to install {productname} with the {offering1} listing in the {market}.

From a single ephemeral appliance, you launch a three-node EKS cluster in a geography of your choosing, and install {productname} in this cluster using the current rancher-stable release. 
...
With {offering1} you instantiate an ephemeral appliance from which you deploy {productname} into a new {eks_long} cluster.
...
the instance that run the Rancher cluster can be resized when needed.
====
...
[NOTE]
====
The __Clean resources__ function gives you a mechanism for removing your Rancher environment.
...
This instantiates the {offering3} status for your deployment.
{offering3} is outlined here https://www.rancher.com/products/rancher[Rancher Prime].

If you procured {offering2}} via the AWS Marketplace you can get support by generating a 'supportconfig' and uploading it via the SUSE Customer Center (SCC) at scc.suse.com.
