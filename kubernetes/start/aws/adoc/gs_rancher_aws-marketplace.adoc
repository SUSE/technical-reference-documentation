...
. Create a new Role named 'RancherSetupEC2role' of 'Use Case' type 'EC2', and attach the policy 'RancherSetupEC2policy' to it. This role will be attached to the deployment automation EC2 instance once it is created. 
+ 
...
. In the AWS Console, navigate to EC2 > Instances and select the radio button for the newly created instance  which corresponds to the AMI ID, and name this instance, so you can find it later.
+ 
image::aws-mp-rancher-10.png[DEFINITION, scaledwidth="85%", align="center"]

. With the instance selected, navigate to 'Actions' > 'Security' > 'Modify IAM Role' to select the role 'RancherSetupEC2role' and 'Update IAM role'. The instance now has permissions to deploy EKS and install SUSE Rancher using HELM. 
+ 
image::aws-mp-rancher-12.png[DEFINITION, scaledwidth="85%", align="center"]

. After your instance is in the _running_ state, use your browser to connect to the public IP address of the instance using the https The instance IP address is found in the instance overview of the AWS web console. The server certificate is self-signed and you need to accept the security exception in your browser. 
+ 
...
. The username for the authentication is the EC2 instance ID and the password is the AWS account ID without dashes. Instance ID and account ID can be obtained from the AWS console.
+ 
...
. Policy Simulator verifies all permissions. If an error occurs at this sage, then the Role was not correctly applied to the instance.
Click Next. 
+ 
#image::aws-mp-rancher-.png[DEFINITION, scaledwidth="85%", align="center"]
