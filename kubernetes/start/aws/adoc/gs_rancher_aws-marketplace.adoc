...
+ 
image::aws-mp-rancher-01.png[AWS login, scaledwidth="65%", align="center"]
...
. Open an additional browser window or tab and navigate to the {offering} in the {market} at https://aws.amazon.com/marketplace/pp/prodview-go7ent7goo5ae[{offering}], then click __Continue to Subscribe__.
+ 
...
. Select the Region matching your newly created VPC, then click __Continue To Launch__.
+ 
image::aws-mp-rancher-07.png[Select VPC, scaledwidth="75%", align="center"]

. Select the VPC, Subnet, Security Group, and Key Pair you created previously, then __Launch__.
+ 
...
. Obtain the username and password for the instance.
+
[TIP]
====
The username is the EC2 Instance ID and the initial password is the AWS account ID without dashes.
...
. Return to the {offering} UI and click __Login__.

. Use the username and password you just obtained to log in.
+ 
...
. Choose a region for deploying your {eks_long} management cluster, the click __Next__. 
+ 
...
. Enter the fully qualified domain name (FQDN) for your Rancher management server, then click __Next__.
//
+
[NOTE]
====
...
. Take note of the URL at which {productname} will be served, then click __Next__ to begin deployment of Rancher.
+ 
...
[NOTE]
====
The __Clean resources__ function gives you a mechanism for restoring your Rancher environment.
...
+
image::aws-mp-rancher-31.png[Rancher - Create Cloud Credential, scaledwidth="85%", align="center"]
Deploy or import downstream clusters and node-groups for workloads.    
+
image::aws-mp-rancher-32.png[Rancher - Create EKS Cluster, scaledwidth="85%", align="center"]


== Priority Support

// Create a correct description of the support offering.
This offer turns an instance of Rancher (Rancher Manager) running in AWS into 'Rancher Prime', which is a SUSE supported product, by installing a billing and metering container.The container will validate the support entitlement purchased through the AWS Marketplace against the quantity of downstream cluster nodes running in the environment.
This billing and metering container will provide the ability to generate supportconfigs which enable the user to open support tickets in the SUSE Customer Center(SCC).
The supported node count (example 20, 40, or 60 nodes) does not include the 3-node EKS cluster deployed during 'Rancher Setyp', which is supported for free.
The downstream cluster nodes can be located anywhere, and multiple clusters, geographies, datcenters, edge devices, and clouds are supported.

You can also purchase 'Priority Support' for your {platform1} implementation, by contacting SUSE directly at aws@suse.com or purchasing it via the {market} offerings for your Geography.

image::aws-mp-rancher-34.png[Rancher Priority Support offerings, scaledwidth="85%", align="center"]

You can also purchase 'Priority Support' as a 'Private Offer' via your normal AWS or SUSE sales channel or by contacting aws@suse.com.
A 'Private Offer' allows you to purchase support for nodes in any increment.
...
You can also reference this https://www.youtube.com/watch?v=9dznJ7Ons0M[step-by-step walk-through video].
...
* https://aws.amazon.com/marketplace/search?searchTerms=SUSE+Rancher&CREATOR=1595dc13-315b-4349-8640-bf36795ae787%2Cd8e49cc8-b2f8-448d-8531-fc8e873121b0&filters=CREATOR[{platform1} products including 'Priority Support'].
