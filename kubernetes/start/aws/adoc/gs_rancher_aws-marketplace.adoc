...
image::logo-lockup_suse-aws_vert_dark.svg[SUSE | Amazon Web Services, scaledwidth="50%", align="center"]
...
In this guide, you learn how to install {productname} with the {offering1} listing in the {market} listing.
...
{platform1} is available in the {market} as the {offering1} listing.
With {offering1} you instantiate a management node from which you deploy {productname} into a new {eks_long} cluster.

In this section, you use the {offering1} listing in the {market} to install {platform1}.
...
. Open an additional browser window or tab and navigate to the {offering1} in the {market} at https://aws.amazon.com/marketplace/pp/prodview-go7ent7goo5ae[{offering1}], then click __Continue to Subscribe__.
+ 
...
Elevate your Rancher landscape to _{productname2}_ with _{offering2}_ in the {market}.
https://ranchermanager.docs.rancher.com/getting-started/quick-start-guides/deploy-rancher-manager/prime[{productname2}] enables you with priority support, security assurances, extended lifecycles, access to focused architectures and Kubernetes advisories, and more.

Purchase {productname2} via the {market} with the {offering2} for your geography.

image::aws-mp-rancher-34.png[Rancher Priority Support offerings, scaledwidth="85%", align="center"]

You can also purchase {productname2} via your normal AWS or SUSE sales channel or by contacting aws@suse.com directly.

When activated, a billing and metering container runs in your environment that validates the support entitlement purchased through the {market} with the quantity of managed, downstream cluster nodes.
You can then generate _supportconfigs_ and open support tickets in the SUSE Customer Center (SCC).
