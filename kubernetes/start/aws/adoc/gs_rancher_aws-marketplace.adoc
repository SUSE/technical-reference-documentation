...
* Route53 registered domain under the currently logged in AWS account. If leveraging a Route53 domain from a different account, please refer to https://docs.aws.amazon.com/IAM/latest/UserGuide/tutorial_cross-account-with-roles.html[Delegate access across AWS accounts]
...
The automated deployment version of {productname} may vary from the latest release. You can upgreade to the latest version of {productname} using HELM by following this https://ranchermanager.docs.rancher.com/getting-started/installation-and-upgrade/install-upgrade-on-a-kubernetes-cluster/upgrades[guidance]
