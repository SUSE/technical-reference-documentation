...
This offer turns an instance of Rancher (Rancher Manager) running in AWS into a supported offering by installing a billing and metering container. The container will validate the support entitlement purchased through the AWS Marketplace against the quantity of nodes running in the environment. This container will provide the ability to generate supportconfigs which enable the user to open support tickets in the SUSE Customer Center. The downstream nodes can be located anywhere.

To purchase 'Priority Support' for your {platform1} implementation, you can contact SUSE directly at aws@suse.com or purchase it via the {market} offerings for your Geography.
