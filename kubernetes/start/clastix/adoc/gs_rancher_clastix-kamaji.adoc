...
=== Verify Kamaji Operator and Kamji Console
...
[IMPORTANT]
====
Tenant control plane pods are exposed by a LoadBalancer service that is the 'ControlPlaneEndpoint' for the worker nodes.
...
CLASTIX Kamaji delivers a highly scalable and high-density Kubernetes control plane infrastructure with reduced operational overhead, yielding faster deployment, configuration, upgrades, and maintenance.
