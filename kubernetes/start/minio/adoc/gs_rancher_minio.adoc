...
image::gs_rancher_minio_architecture.png[General Architecture, scaledwidth="95%", align="center"]
...
MinIO Operator Console:: This service provides a rich user interface to ease management of MinIO object storage.  The MinIO Operator Console is installed and configured automatically with the MinIO Operator.
...
image::rancher_minio_architecture_distributed_001.png[Distributed architecture, scaledwidth="85%", align="center"]
...
[TIP]
====
Be sure drives are provisioned in JBOD (Just a Bunch of Disks) mode with no RAID, ZFS pooling, or other redundancy, resiliency, or tooling layers.
...
If not already installed on your system, https://krew.sigs.k8s.io/docs/user-guide/setup/install/[install Krew] before proceeding.
...
In a Kubernetes environment, provisioning large stateful services requires more care as the full lifecycle and behavior of pods, networking, and other transient resources come into play.
...
. Log into the SUSE Rancher UI and select the cluster where you intend to deploy and use MinIO object storage.
...
You can modify this during "Install: Step 1" or just click _Next_ to continue.
+
...
. Verify that you see 'minio-operator' and 'console' pods in the 'Running' stateas well as the 'service/operator' and 'service/console' assigned to cluster IP addresses.
...
----
+
where <TOKEN> is your unique https://en.wikipedia.org/wiki/JSON_Web_Token[JSON Web Token (JWT)] that you must use to log into the MinIO Operator Console.
...
Kustomize does this by referencing a https://kubectl.docs.kubernetes.io/references/kustomize/glossary/#kustomization[kustomization], which can be in the form of a single file ('kustomization.yaml'), a directory containing that file along with others, a tarball of the directory, a git archive, etc.
