...
MinIO is a Kubernetes-native object store designed to provide high performance through a familiar, S3-compatible API.
Each MinIO tenant represents an independent MinIO object store within the Kubernetes cluster.

image::gs_rancher_minio_architecture.svg[scaledwidth="75%", align="center"]


The MinIO Operator automatically generates the necessary PersistentVolumeClaims (PVCs) for the tenant.
The Kubernetes cluster must have sufficient PersistentVolumes on the worker nodes for the tenant to start up successfully.  

While Kubernetes supports a variety of storage options, MinIO strongly recommends using direct-attached storage that is local to the workers on which MinIO pods run.
Specifically, the Kubernetes cluster should have Persistent Volumes associated with that direct-attached storage.
...
For other PV types, defer to the documentation for that PV. 
...
To manually provision local persistent volumes, format the underlying drives as XFS and apply an identifiable drive label. Mount drives in `/etc/fstab` using drive label to ensure drive ordering does not change if the host machine restarts. 
