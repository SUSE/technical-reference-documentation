...
After completing these steps, you are able to access the Kubeflow user interface through your Web browser.
...
. Log into the Kubeflow user interface (UI) with the credentials:
+
--
...
Some things that could prevent connecting or logging into the Kubeflow UI include:
...
Moreover, many Kubeflow components leverage this cryptographic protocol for secure cookies and may not function at all without it.
...
. Log into the Kubeflow UI with the credentials:
+
--
* Email address: `user@example.com`
...
[IMPORTANT]
====
Proceed to the next section only after you have verified that you can connect to and log into the Kubeflow UI.
====
...
You can configuring cert-manager for https://cert-manager.io/docs/configuration/acme/[Automated Certificate Management Environment (ACME)] issuers, like Let's Encrypt.
...
. Verify that the hostname for the certificate resolves correctly.
+
[source, console]
----
getent hosts ${FQDN}
...
In this guide, you used {product3_full} to provision nodes to host a {product2} Kubernetes cluster, then installed, configured, and secured Kubeflow so it could be managed by {product1_full}.
