...
//
+
This system is used to access the cluster.
//
+
{product4_full} ({product4}) is recommended.
+
[NOTE]
====
{product4} {product4_version} is used for this guide.
====
+
...
* {product1} environment
//
+
{product1} is used to deploy the five virtual machines for Kubernetes nodes.

** {product1} namespace
//
+
All nodes will be in the same {product1} namespace.

** Networking
//
+
All nodes will be connected to a single VLAN with DHCP, DNS, and routing to the Internet.

** Control plane and etcd pool nodes
//
+
Each node will be configured with 4 vCPUs, 8 GB of RAM, and a 40 GB storage volume.

** Workload nodes
//
+
Each workload node will be configured with 8 vCPUs, 16 GB of RAM, and a 40 GB storage volume.

** Operating system
//
+
All nodes will be configured with {product4} {product4_version} minimal, using the QCOW2 image with cloud-init enabled (previously known as the OpenStack image).
+
[IMPORTANT]
====
...
[NOTE]
====
The node sizings used here are for basic testing purposes only.
...
[NOTE]
====
Authors: Organize the following into ordered steps.
====
