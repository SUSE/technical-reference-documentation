...
This document provides a procedure for installing Kubeflow on SUSE Rancher. The Kubeflow project provides a straightforward way to deploy open-source machine learning (ML)  tools to Kubernetes clusters, regardless of the underlying infrastructures. SUSE Rancher enables efficient management of these clusters.
...
Deploying Kubeflow on Kubernetes clusters managed by SUSE Rancher requires a number of manual steps. 
...
This guide will help you take the first steps to deploy Kubeflow on Kubernetes clusters managed by SUSE Rancher.
...
This document is intended for developers and system administrators who are responsible for facilitating machine learning (ML) workflows on Kubernetes clusters.
...
In addition to Kubeflow itself, there are a number of supporting components that are required for this solution. We will walk through the process of installing each of them.
...
We begin by using the Rancher UI to create a project in Harvester named kubeflow-on-harvester that contains a namespace named kubeflow-cluster. The cluster name should be set to: 
[source, console]
----
...
The Kubernetes `Cluster Configuration` should be configured as follows:

* On the __Basic__ tab:
** Select Kubernetes version v1.24.9+rke2r2 
+
NOTE: This version is currently deprecated but is needed for Harvester Cloud Provider support. If you are not using Harvester, you can select a newer version.
...
After the cluster has been created, you should verify and reboot the RKE2 nodes
...
IMPORTANT: Does this need to be added?
...
In this guide, you learned how to deploy Kubeflow on Kubernetes clusters managed by SUSE Rancher.
