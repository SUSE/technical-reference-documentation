...
This document provides guidance for deploying Kubeflow onto a {product2} Kubernetes cluster managed by {product1}.
This guide also illustrates provisioning virtual machine hosts using {product3_full}.
...
To be successful with this guide, you should have basic knowledge of Kubernetes operations in a {product1} environment as well as standard Linux command line container management with Kubectl, Helm, and Kustomize.
...
. Provision virtual machines for your Kubernetes cluster nodes with {product3}.

. Instantiate and configure a {product2} cluster with {product1}.
...
[NOTE]
====
Kubeflow can be deployed to any {product2} cluster, regardless of how it is provisioned.
{product3_full} is used in this guide to provision virtual machines for the {product2} cluster nodes, along with https://{product5_url}[{product5_full}] for persistent storage.

If you already have your target {product2} cluster, be sure you also provision persistent storage (such as https://{product5_docs}[Longhorn].
====
...
If you would like to try a different version, select it from the https://{kubeflow_url_repo}/manifests/releases[available releases] and update the above command with the appropriate release tag.
====
...
. After verifying that you can log into the Kubeflow UI, you can simply close your browser.
...
Some things that could prevent connecting or loggging into the Kubeblow UI include:
...
Many Kubeflow components use secure cookies.
This means that accessing Kubeflow with HTTP over a non-localhost domain (for example, via MetalLB) does not work.
To overcome this, you need to configure https://istio.io/[Istio] (which was installed with Kubeflow) to use MetalLB and to enable the https://istio.io/latest/docs/reference/config/networking/gateway/[Istio Gateway] to use the HTTPS protocol. 

You will need a signed TLS certificate.
If you do not have one, you can get one for free with https://letsencrypt.org/[Let's Encrypt].
...
One of the challenge methods uses https://letsencrypt.org/docs/challenge-types/#dns-01-challenge[DNS-01].
...
. Update AWS Route53 (or the DNS provider you use) wih the Kubeflow IP address and the desired fully qualified domain name (FQDN) for the Kubeflow UI.
...
[IMPORTANT]
====
Proceed to the next section only after you verify that you can connect to and log into the Kubeflow UI.
====
