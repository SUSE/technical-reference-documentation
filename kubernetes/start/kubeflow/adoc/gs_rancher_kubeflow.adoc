...
[NOTE]
====
*Mark*: This section could use further formatting as numbered steps.
The first step (creating a project in Harvester) needs to be explained better, as we can't expect the reader to know how to do this.
====

In this guide, {product3} is used to provision virtual machines for your cluster nodes.
...
====
*Mark*: 
I haven't moved this to a separate file yet.
The IP address pools section was a bit confusing because of the options provided.
I simplified by not mentioning the CIDR option.
Could we illustrate with real IP addresses and say something like, "replace the addresses shown in the following command with the appropriate addresses for your environment"?
I don't know the answer to this question: The test procedure mentions nginx. Is this appropriate to our case?
====
...
Specify the starting and ending IP address for this default pool by replacing `aa.bb.cc.dd` and `ee.ff.gg.hh` in the following commands:
+
[source, console]
----
export DEFAULT_IP_RANGE_START=aa.bb.cc.dd
export DEFAULT_IP_RANGE_END=ee.ff.gg.hh
----
+
For example:
+
...
====
*Mark*:
I didn't get back to this section.
We need to clarify that we are showing the single-command installation method that works at the time the document was written but that the reader should review the Kubeflow Installation guide for the latest updates.
====
...
====
*Mark*:
As Alex noted on Tuesday's call, the port-forwarding (https://github.com/kubeflow/manifests#port-forward) works because all communication is essentially internal to the cluster.
This is of no real value.
I think we will be asked to detail the LoadBalancer method, but, as pointed out on the call, we don't know how to do this yet.
====
