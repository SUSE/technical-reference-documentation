...
:usecase: Kubernetes solution ideal for critical data workloads, like Database-as-a-Service
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
{platform1} and {otherproduct1} provide you with a complete cloud and Kubernetes native technical stack that does not compromise on the key features modern developers and businesses expect in today's data driven environment.
...
This guide walks through the basic steps for deploying {otherproduct1} with {platform1} and demonstrates the high availability use case with a stateful application.
...
{otherproduct1} is designed to feel familiar to Kubernetes users: storage is managed through standard StorageClasses and PersistentVolumeClaims, and features are controlled by native Kubernetes Labels and selectors, prefixed with _storageos.com/_.
...
Installing {otherproduct1} into your {productname} Kubernetes cluster is easy.
The two recommended methods are:

. Through https://rancher.com/docs/rancher/v2.6/en/helm-charts/[Rancher Apps catalog] using the https://github.com/rancher/partner-charts/tree/main/charts/ondat-operator/ondat-operator[Ondat Helm Chart].

. With the https://docs.ondat.io/docs/reference/kubectl-plugin/[Ondat kubectl plugin].


In this guide, you use the {otherproduct1} `kubectl` plugin to install {otherproduct1} and deploy an etcd cluster operator in your {productname} Kubernetes cluster.


. Install local path provisioner.
+
By default, a newly provisioned {productname} Kubernetes cluster does not have a deployed CSI driver.
+
...
+
Run the following `kubectl-storageos` plugin command to conduct preflight checks against the {productname} Kubernetes cluster to validate that key {otherproduct1} prerequisites have been met before proceeding.
+
[source, bash]
----
# Conduct preflight checks.
...
+
Run the following `kubectl-storageos` plugin command to install {otherproduct1}.
+
[source, bash]
----
# Install Ondat.
...
. simulating a node failure scenario to demonstrate how {otherproduct1}'s replication feature can to quickly fail over to a new replica volume when a master volume suddenly goes permanently offline.
...
With Ondat and {platform1}, you can create a Kubernetes native stack that ensures your critical, stateful workloads are highly available and durable.
