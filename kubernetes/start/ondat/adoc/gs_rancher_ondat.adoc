...
image::rancher_ondat_arch-diagram-high-level.png[scalewidth="85%", align="center"]
...
image::rancher_ondat_arch-diagram-components.png[scalewidth="85%", align="center"]
...
For this demonstration, you will deploy MongoDB as the stateful application.  The https://github.com/percona/percona-postgresql-operator[MongoDB operator] is developed and maintained by Percona.

. Run the following __kubectl__ commands to deploy the  operator:
+
[source, bash]
----
...
This is the TAP feature in action, which is leveraging the custom regions node labels that were created in the previous step.
...
NOTE: The nodes where the volumes reside may be different for your cluster.  Ensure that you substitute the correct node names in the commands.
...
In this example output, notice that Ondat elected the replica volume on node __demo-worker-node-3__  to host the master volume since __demo-worker-node-2__ no longer exists.
Also notice that a new replica volume was automatically created on node __demo-worker-node-5__ to ensure that the replica count remains at two, as defined.
