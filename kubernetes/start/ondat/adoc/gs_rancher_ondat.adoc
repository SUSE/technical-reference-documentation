...
In this guide, you use the {otherproduct1} `kubectl` plugin to install {otherproduct1} and deploy an etcd cluster operator in your Kubernetes cluster with {productname}.
...
+
Run the following `kubectl-storageos` plugin command to conduct preflight checks against the Kubernetes cluster to validate that key {otherproduct1} prerequisites have been met before proceeding.
+
[source, bash]
----
# Conduct preflight checks.
