...
{comp2} is deployed to a {comp1} cluster, using the underlying RKE2 Kubernetes cluster for the control plane, and includes NGNIX ingress to serve the {comp2-short} user interface (UI).
...
[NOTE]
====
This will install {comp2-short} with basic authentication, with a username of ‘kasten’ and password ‘Kasten/4u!’.
...
The {comp2-short} dashboard should load and ask for a username and password.
...
To do so, navigate to _Applications_, click the menu to the right of a namespace you wish to protect, and select _Create_ _Policy_.
+
...
[NOTE]
====
Watch this https://youtu.be/CrkbHHzdvv4?si=a66O8u_ROXSpMeQ9[step-by-step visual walkthrough] of deploying {comp2-short} for Kubernetes into a {comp1} environment, creating a backup policy, and performing a backup and restoration of a VM.
====
