...
Protecting an application with K10, usually accomplished by creating a policy, which requires understanding three concepts:

* Snapshots and Backups: Data capture mechanisms - depending on your environment and requirements, you might need just one or both
* Scheduling: Specification of application capture frequency and snapshot/backup retention objectives
* Selection: Identification of the applications protected by a policy - resource filtering can be used to restrict what is captured on a per-application basis

This section demonstrates how to use these concepts in the context of a K10 policy to protect applications.
Today, an application for K10 is defined as a collection of namespaced Kubernetes resources (e.g., ConfigMaps, Secrets), relevant non-namespaced resources used by the application (e.g., StorageClasses), Kubernetes workloads (e.g., Deployments, StatefulSets, standalone pods, etc.), deployment and release information available from Helm v3, and all persistent storage resources (e.g., PersistentVolumeClaims and PersistentVolumes) associated with the workloads.

While you can always create a policy from scratch from the Policies page, the easiest way to define policies for unprotected applications is to click on the __Applications__ card on the main dashboard.
This will take you to a page where you can see all applications in your Kubernetes cluster.
+
...
To protect any unmanaged application, simply click __Create a policy__ and, as shown below, this will take you to the policy creation section with an auto-populated policy name that you can change.
The concepts highlighted above will be described below in the context of the policy creation workflow.
+
...
They are usually used in the context of disk volumes (PVC/PVs) used by the application but can also apply to application-level data capture (e.g., with Kanister).
...
However, storage snapshots usually also suffer from constraints such as having relatively low limits on the maximum number of snapshots per volume or per storage array.
...
NOTE: A number of public cloud providers (e.g., AWS, Azure, Google Cloud) actually store snapshots in object storage, and they are retained independent of the lifecycle of the primary volume.
However, this is not true of all public clouds (e.g., IBM Cloud), and you might need to enable backups in public clouds for safety.  Please check with your cloud provider's documentation for more information.
...
Care should be taken with more general-purpose workloads because of the risk of stressing underlying storage infrastructure or running into storage API rate limits.
...
By default, backups performed via exports will be set up to export every snapshot into a backup.
However, it is also possible to select a subset of snapshots for exports (e.g., only convert every daily snapshot into a backup).
