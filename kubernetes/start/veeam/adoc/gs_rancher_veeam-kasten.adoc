...
To enable basic authentication, you will first need to generate https://httpd.apache.org/docs/2.4/programs/htpasswd.html[htpasswd] credentials by either using an online http://www.htaccesstools.com/htpasswd-generator/[tool] or via the htpasswd binary found on most systems.
...
Alternatively, you can use an existing secret that contains a file created with htpasswd.
...
NOTE: It is also possible to perform an interactive walkthrough of the K10 dashboard via a Guided Tour.
The tour is available when the K10 dashboard is accessed for the first time or via the Dashboard option on the https://docs.kasten.io/latest/usage/overview.html#k10-settings[Settings] page.
...
* Compliant: Objects to which both policies apply and the policy SLAs are being respected


*Applications, Namespaces, and Workloads*

The K10 platform by default equates names to applications for ease of use and consistency with Kubernetes best practices, use of RBAC, and to mirror the most common application deployment pattern.
...
Assuming you have already installed applications, clicking on the Applications card on the dashboard will take you to the following view.
+
...
NOTE: If you click on the ___Compliant__, __Non-Compliant__, or __Unmanaged buttons__, it would automatically filter the view for you.
...
Within K10, policies are used to automate your data management workflows.
Going back to the main dashboard, you will find a section on how to manage policies right next to the __Applications__ card.
To achieve this, policies combine actions you want to take (e.g., snapshot), a frequency or schedule for how often you want to take that action, and selection criteria for the resources you want to manage.
+
...
If you click the __Policies__ card, you will notice that no default policies are created at install time, but a policy can be either created from this page or from the application page shown earlier.


=== Creating a Location Profile 

K10 can usually invoke protection operations such as snapshots within a cluster without requiring additional credentials.
While this might be sufficient if K10 is running in some (but not all) of the major public clouds and if actions are limited to a single cluster, it is not sufficient for essential operations such as performing real backups, enabling cross-cluster and cross-cloud application migration, and enabling DR of the K10 system itself.
...
NOTE: When certain cloud providers (e.g., AWS) are selected, provider-specific options (e.g., IAM Roles) will appear for configuration, if needed.
...
To protect any unmanaged application, simply click __Create New Policy__ and, as shown below, this will open a New Policy dialog.
+
...
The key policy concepts noted above are described below in the context of the policy creation workflow.


=== Snapshots and Backups

All policies center around the execution of actions.  You start by selecting the snapshot action with an optional backup (currently called an export).
...
Snapshots are the basis of persistent data capture in K10.
...
It is therefore highly recommended that you create backups of your application snapshots too.

NOTE: A number of public cloud providers (including AWS, Azure, and Google Cloud) actually store snapshots in object storage, and they are retained independent of the lifecycle of the primary volume.
However, this is not true of all public clouds, and an independent backup would provide essential safety.  Please check with your cloud provider's documentation for more information.
...
Actions can be set to execute at an hourly, daily, weekly, monthly, or yearly frequency, or on demand.
By default, actions set to hourly will execute at the top of the hour and other actions will execute at midnight UTC.

It is also possible to select the time at which scheduled actions will execute and sub-frequencies that execute multiple actions per frequency.
See https://docs.kasten.io/latest/usage/protect.html#advanced-schedule-options[Advanced Schedule Options].

Sub-hourly actions are useful when you are protecting mostly Kubernetes objects or small data sets.
Care should be taken with more general-purpose workloads due to the risk of stressing the underlying storage infrastructure or running into storage API rate limits.
Further, sub-frequencies will also interact with retention (described below).
...
By default, backups performed via exports will export every snapshot into a backup.
However, it is also possible to select a subset of snapshots for exports.
For example, you could convert only daily snapshots into backups.
...
With this backup rotation scheme, hourly snapshots and backups are rotated on an hourly basis with one graduating to daily every day and so on.
It is possible to set the number of hourly, daily, weekly, monthly, and yearly copies that need to be retained, and K10 will take care of cleanup at every retention tier as well as graduation to the next one.
...
By default, backup retention schedules will be the same as snapshot retention schedules, but these can be set to independent schedules if needed.
This allows users to create policies where a limited number of snapshots are retained for fast recovery from accidental outages while a larger number of backups are stored for long-term recovery needs.
...
Any artifacts created by manual policy runs will need to be cleaned up.
...
By default, actions set to hourly will execute at the top of the hour and other actions will execute at midnight UTC.
If you require finer control of timing, you can find this in __Advanced Options__.
For example, if the Action Frequency is set to Daily, you can specify the hour of the day and the minutes after the hour when the action will start as well as whether to use UTC or local time.
