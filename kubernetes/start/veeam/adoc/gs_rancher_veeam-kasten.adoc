...
Once you have created a policy and have navigated back to the main dashboard, you will see the selected applications quickly switch from unmanaged to non-compliant (that is, a policy covers the objects but no action has been taken yet).i
...
While the UI uses the __Export__ term for backups, no import policy is needed to restore from a backup.
