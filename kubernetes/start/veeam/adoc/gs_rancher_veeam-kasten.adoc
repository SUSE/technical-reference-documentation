...
This guide provides an overview of the steps to install and set up {otherproduct1} in your {platform1} Kubernetes environment and to perform a simple backup and restore of an application.
...
This provides enterprise operations teams the flexibility to choose the deployment environments that best meet their needs - on-premises, public cloud, and hybrid.
...
----
+
In this case, you need to create a role binding or cluster role binding for the account to ensure that it has the appropriate permissions for {otherproduct1short}.
To learn more about permissions, see https://docs.kasten.io/latest/access/authorization.html#authz[Authorization].


=== Kasten {otherproduct1short} dashboard overview
...
[TIP]
====
A guided tour of the dashboard is available when the {otherproduct1short} dashboard is accessed for the first time or via the dashboard option on the https://docs.kasten.io/latest/usage/overview.html#k10-settings[Settings] page.
====
...
The {otherproduct1short} platform equates namespaces to applications for ease of use and consistency with Kubernetes best practices, use of role-based authentication controls (RBAC), and to mirror the most common application deployment patterns.
...
{otherproduct1short} can invoke protection operations, such as snapshots, within a cluster without requiring additional credentials, and this may be sufficient if {otherproduct1short} is running in the major public clouds and actions are limited to a single cluster.
...
This section demonstrates how to use these concepts in the context of a {otherproduct1short} policy to protect applications.
...
With {otherproduct1short}, all policies center around the execution of actions.
...
It is possible to set the number of hourly, daily, weekly, monthly, and yearly copies that need to be retained, and {otherproduct1short} will take care of cleanup.
...
By default, the backup retention schedule is the same as the snapshot retention schedule.
...
In {otherproduct1short}, you can specify which applications are bound to a policy by name or label.
...
For example, if you specify 'mysql-*', {otherpoduct1short} will match all applications whose names start with 'mysql-'.
...
When both snapshots and backups are present, the {otherproduct1short} provides you the option to select the instance you want to use to restore the application.
