...
Public, or simply widely accessible, APIs (Application Programming Interfaces) open up a world of retrieving data and automating tasks that simply cannot be made available through normal command line or graphical interfaces. They allow us to access and interact with a huge number of web services in ways that we define ourselves. In this guide, we'll explore how to capture API calls, reformat them to our needs, and run them against the Rancher Prime API server using Linux `curl` commands, an essential skill for any developer or system administrator.

== Prerequisites

Before we dive into the world of API interactions, let's ensure you have everything you need:

1. *Linux Environment*: This guide assumes you are using a Linux-based operating system. You can use a Linux distribution like Ubuntu, CentOS, or even a Linux subsystem on Windows.

2. *Terminal*: You'll need a terminal application for running `curl` commands. Most Linux distributions come with a terminal pre-installed.

3. *`curl` Installed*: Check if `curl` is already installed on your system by running `curl --version` in your terminal. If not, you can install it using your system's package manager (e.g., `apt`, `yum`, or `brew`).

4. *Rancher Prime API Server*: Ensure you have access to the Rancher Prime API server and the necessary authentication credentials.
...
Let's put theory into practice by making a sample request to the Rancher Prime API server. In this example, we'll retrieve information about clusters. We'll also learn how to capture the Network XHR output using a web browser's "Inspect" function to use with `curl` commands.
