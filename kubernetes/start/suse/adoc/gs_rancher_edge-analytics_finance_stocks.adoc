...
:title: Edge Analytics with SUSE Rancher - Finance - Market Predictions : Getting Started
:productname: SUSE Rancher 2.5
:productversion: 2.5
:productname2: K3s v1.20.14+k3s1
:productversion2: v1.20.14+k3s1
:partnerproductname: Fin-Dashboard
:pythonname: Python 3.10
:author1: Navin Chandra, Google Summer of Code Contributor, openSUSE Project
...
image::suse-rancher-logo-green.svg[SUSE Rancher, scaledwidth="65%", align="center"]
...
This guide illustrates the facility of Kubernetes to support edge analytics in the financial sector by using SUSE Rancher and K3s to deploy a machine learning model for stock price predictions and a web-based front-end for user interaction and visualization.

* https://www.suse.com/products/suse-rancher/[SUSE Rancher] is an enterprise Kubernetes and container management platform that allows you to deploy workloads anywhere - at the data center core, in the clouds, and even at the edge - efficiently and with enterprise-grade security and support.

* https://k3s.io/[K3s] is an open source, lightweight, certified Kubernetes distribution built for IoT and edge computing.
...
This document is intended for DevOps, MLOps, developers and data scientists who want to containerize their ML workloads and deploy them at the edge.
...
* A https://rancher.com/docs/rancher/v2.5/en/[Rancher] environment.
//
+
This guide uses {productname}.
Subsequent minor releases should work with little to no modification.

* A Kubernetes cluster managed by Rancher.
//
+
This guide leverages {productname2}.
+
[NOTE]
====
K3s is an ideal Kubernetes distribution for edge deployments, but, with SUSE Rancher, you can manage any https://www.cncf.io/certification/software-conformance/#logos[CNCF-certified Kubernetes distribution].
====
...
In this guide, you use Rancher to deploy {partnerproductname}, a stock prediction web application, which you can obtain from the project's https://github.com/navin772/GSOC_Stock_prediction/tree/master/fin_dashboard[code repository].
{partnerproductname} is discussed in more detail later in this guide.
...
//
+
In this guide, you use https://kubectl.docs.kubernetes.io/references/kustomize/[Kustomize].


//

Some additional components and tools used in this guide or by {partnerproductname} include:
...
* https://streamlit.io/[Streamlit v1.11.0] - to build a web-based, interactive front-end for the ML model.
...
In this section, you explore a subset of the {partnerproductname} code by creating a https://jupyterlab.readthedocs.io/en/stable/[Jupyter notebook] that illustrates how data is accumulated and processed, a model is created and used, and model predictions are visualized.
...
In addition to the ML analytics workload, {partnerproductname} also provides a web-based front-end to support user interaction.

=== Features

The front-end application features useful functionality for the end-user, including:
...
{partnerproductname} is composed of the following Python modules:
...
* https://github.com/navin772/GSOC_Stock_prediction/blob/master/fin_dashboard/index_correlation.py[index_correlation.py] - Correlates the stock with it's parent index.

* https://github.com/navin772/GSOC_Stock_prediction/blob/master/fin_dashboard/main.py[main.py] - Contains all the Streamlit configurations/page setup and utilizes the other files to create the application.
...
. Clone the {partnerproductname} code from GitHub into an empty directory.
+
[source, bash]
----
...
You define the base container image and the contents of the layers in a plain text file, called a https://docs.docker.com/engine/reference/builder/[Dockerfile]. Once this is done, you simply call the `docker build` command to create the new container image.
...
. Tag the image to your DockerHub account.
+
[source, bash]
----
...
. Push the image to your https://hub.docker.com/[DockerHub] account.
//
+
...
[TIP]
====
Be sure you log into your https://hub.docker.com/[DockerHub] account before pushing the image.
...
With your manifest files defining the deployment, you just need to execute the deployment to your Kubernetes cluster.
...
. Once the deployment is complete, note the 'CLUSTER-IP' and 'PORT(S)' associated with 'service/stock-analysis'.
//
+
...
Get started using {partnerproductname}:

. Open your web browser to the IP and port of the service (e.g., http://10.43.148.169:32169).

. The Dashboard automatically detects your country, but you can select another from the drop down.
+
...
In this guide, you learned how to containerize an ML application and deploy it into a Kubernetes environment managed by SUSE Rancher.

Below are a few resources to help you continue your exploration of SUSE Rancher and K3s for edge analytics:
