...
image::rancher-suse-logo-horizontal-color.svg[Rancher by SUSE, scaledwidth="65%", align="center"]
...
This guide illustrates the facility of Kubernetes to support edge analytics in the financial sector by using Rancher by SUSE and K3s to deploy a machine learning model for stock price predictions and a Web-based front-end for user interaction and visualization.

* https://www.suse.com/solutions/enterprise-container-management/#rancher-product[Rancher by SUSE] is an enterprise Kubernetes and container management platform that allows you to deploy workloads anywhere - at the data center core, in the cloud, and even at the edge - efficiently and with enterprise-grade security and support.
...
* A https://ranchermanager.docs.rancher.com/v2.6[Rancher] environment.
//
+
This guide uses {platform1}.
Subsequent minor releases should work with little to no modification.

* A Kubernetes cluster managed by Rancher.
//
+
This guide leverages {k8s}.
+
[NOTE]
====
K3s is an ideal Kubernetes distribution for edge deployments, but, with Rancher, you can manage any https://www.cncf.io/certification/software-conformance/#logos[CNCF-certified Kubernetes distribution].
====
...
Following this guide, you use Rancher to deploy {otherproduct1}, a stock prediction Web application, which you can obtain from the project's https://github.com/navin772/GSOC_Stock_prediction/tree/master/fin_dashboard[code repository].
{otherproduct1} is discussed in more detail later on.
