...
[NOTE]
====
The {slm-long} OS image must be downloaded from the {scc-website}[SUSE Customer Center] or {slm-downloads}[SUSE {slm} Downloads page], and it must be located under the `base-images` directory.
====

// This step is confusing, since this is the first mention of a KIWI ISO image and no real instructions on how to generate it.  It also begs the question of why the SL Micro image needed to be downloaded.  Clarify here is required.
. Add your KIWI ISO image (output from {kiwing-docs}[KIWI]) as a base-image in the base-
images folder.
To do that you can follow the steps in the https://documentation.suse.com/suse-edge/3.4/html/edge/guides-kiwi-builder-images.html[full guide instructions in official docs]
...
.. pass:[${ROOT_PASSWORD}] - The root password for the management cluster. This could be
...
.. pass:[${SCC_REGISTRATION_CODE}] - The registration code for the SUSE Customer Center for the
