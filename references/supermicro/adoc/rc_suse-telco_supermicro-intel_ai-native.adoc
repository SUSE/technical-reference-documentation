...
{rt-testing-json}[By configuring a JSON file], you can simulate a particular CPU load percentage and use `cyclictest` to quantify the latency associated with scheduling tasks concurrently.
`cyclictest` can also produce a histogram to visualize the results, as shown below.
For more information about `cyclictest`, see {sle-rt-hw-testing}[SLE RT Hardware Testing].

To provide illustrative, real-time performance, the authors deployed the reference configuration detailed in this document, including the telco tunings provided in the <<Sample Provision file (capi.yaml)>>) and using {smss112-long} hosts.

[NOTE]
====
Host details provided by the `hostnamectl`:
...
This resulted in the plot below, where latency remains below five microseconds for the duration of the test.
