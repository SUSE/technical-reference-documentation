...
+
//
For reference, see <<Example BareMetalHost definition file (bmh.yaml)>>.
...
----
+
For reference, see <<Example BareMetalHost details>>.


. The same information is represented now in the {rancher} UI:
+
image::8.1.4-rancher-ui-1.png[title="{rancher} – Bare Metal Hosts Management View", {rancher} – Bare Metal Hosts Management View, align="center", width=90%]
+
image::8.1.4-rancher-ui-2.png[title="{rancher} – Hardware Data Details for Discovered BareMetalHost", align="center", width=90%]
...
//
+
See <<Example Provision file (capi.yaml)>> for reference.
...
+
image::8.1.4-rancher-ui-4.png[title="{rancher} – Downstream Cluster Nodes in Active State", align="center", width=90%]


== Performance testing

In this section, you conduct performance tests on the cluster described in this document.
to measure the CPU latency in scheduling new tasks on both isolated and housekeeping cores.
You utilize a real-time application, which is a simulator, to replicate specific workloads per CPU during defined time periods.
{json-perf-testing}[By configuring a JSON file], you can simulate a particular CPU load percentage and use `cyclictest` to measure the latency associated with scheduling tasks concurrently.
`cyclictest` quantifies the latency before and after the task scheduling and can produce a histogram to visualize the results.
For more information on cyclictest, see {sle-rt-hw-testing}[SLE RT Hardware Testing]


With the reference configuration detailed in this document (including the Telco tunings shown in the <<Example Provision file (capi.yaml)>>) and providing a CPU load of 95 percent on isolated cores,

Based on the following configurations:

* Kernel args showed in the Provision file (`capi.yaml`)
...
The plot below shows that latency remains below five microseconds for this test.
...
The combination of {stelco-long}, {cpu-long}, and {smss-long}s delivers a future-proof, CPU-only telco-optimized cloud platform that delivers uncompromised performance, radical simplicity, genuine sustainability, and investment protection at scale while dramatically reducing power, physical space, and TCO.
Together, we enable operators to stop compromising and start transforming.
...
To move forward with transforming your network, please contact your {stelco-provider}, {cpu-provider}, or {smss-provider} account team to arrange a joint solution workshop and receive expert sizing assistance tailored to your specific deployment needs.
...
This validated reference architecture confirms that {stelco-long} running on {smss-long}s, powered by {cpu-long} processors can deliver a fully consolidated, CPU-only, telco edge platform capable of supporting AI-native, low-latency, 5G workloads at scale.
The solution is validated to meet stringent telco requirements, demonstrating deterministic sub-5 microsecond latency under high CPU utilization while consolidating vRAN acceleration, AI inference, networking, timing, and security into a single-socket system.
...
==== Example BareMetalHost definition file (bmh.yaml)
...
==== Example Provision file (capi.yaml)
