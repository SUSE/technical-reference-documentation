...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
This document is intended for telecommunications network architects, infrastructure and platform engineers, and operations teams responsible for designing, deploying, and managing 5G and AI-enabled telco edge infrastructure. It is also relevant for technical decision-makers evaluating platform modernization strategies, including IT and network leadership seeking to reduce infrastructure complexity, power consumption, and total cost of ownership.

In addition, this reference architecture is useful for system integrators, solution architects, and partners involved in designing or validating telco edge solutions based on SUSE Telco Cloud, Intel Xeon 6 SoC platforms, and Supermicro telco-optimized hardware.
...
image::media/src/png/intelxeon6SoCoverview1.png[align="center",width=90%]

image::media/src/png/IntelXeon6SoCoverview-2.png[align="center",width=90%]

`Supermicro SYS-112D-42C-FN8P front/back view`:

image::media/src/png/Supermicro-system-front-view.png[align="center",width=90%]

image::media/src/png/Supermicrosystembackview.png[align="center",width=90%]
For more information, see https://www.supermicro.com/de/products/system/iot/1u/sys-112d-42c-fn8p[SYS-112D-42C-FN8P]

`SUSE Telco Cloud Architecture`:

image::media/src/png/SUSE-Telco-Cloud-Architecture.png[align="center",width=90%]
For more information, see the https://documentation.suse.com/suse-edge/3.4/html/edge/atip-architecture.html[SUSE Telco Cloud Architecture] documentation.

`vRAN performance`:


image::media/src/png/vRAN-Performance.png[align="center",width=90%]


//Need to place images
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
...
|===
2+|*Supermicro Compact Telco Edge Server* +
*SYS-112D-42C-FN8P*
...
For more information, see the https://documentation.suse.com/suse-edge/3.4/html/edge/atip-requirements.html#id-hardware documentation.
 ...
 * A container runtime such as Podman or Rancher Desktop

The SL-Micro.x86_64-6.1-Base-GM.raw OS image file must be downloaded from https://scc.suse.com/[SUSE Customer Center] or the https://www.suse.com/download/sle-micro/[SUSE Download page].

=== Network

Follow the official SUSE Edge Telco guidelines and recommendations described in the https://documentation.suse.com/suse-edge/3.4/html/edge/atip-requirements.html#id-network[SUSE Edge ATIP Network Requirements].

=== Ports

For details on port requirements, see the https://documentation.suse.com/suse-edge/3.4/html/edge/atip-requirements.html#id-port-requirements[SUSE Edge ATIP Port Requirements] section of the official SUSE Edge Telco documentation.

=== Services (DHCP,DNS,etc)

For details on supported services such as DHCP, DNS, and others, see the https://documentation.suse.com/suse-edge/3.4/html/edge/atip-requirements.html#id-services-dhcp-dns-etc[SUSE Edge ATIP DHCP/DNS/Services Requirements] section of the official SUSE Edge Telco documentation.

=== Disabling systemd services

For information on disabling unnecessary systemd services, see the https://documentation.suse.com/suse-edge/3.4/html/edge/atip-requirements.html#id-disabling-systemd-services[SUSE Edge ATIP Disabling Systemd Services] section of the official SUSE Edge Telco documentation.

== Edge Image Builder

=== What is Edge Image Builder

The need for easily auditable, reproducible, and customized base images becomes paramount to the long-term stability and maintainability of the environment. https://documentation.suse.com/suse-edge/3.3/html/edge/components-eib.html[Edge Image Builder (EIB)], one of the components of SUSE Telco Cloud, helps mitigate these concerns by providing a mechanism to customize SL Micro base images to include all of the configurations and workload artifacts needed by telco deployments. https://github.com/suse-edge/edge-image-builder[EIB is an open source project] that enables the simplified and quick configuration of customized SUSE Linux Micro images for a variety of scenarios. The resulting image contains all of the configuration and binary artifacts (RPMs, container images, etc.) necessary to provision the node at first boot, providing true zero-touch provisioning even in the case of low-bandwidth connections and air-gapped scenarios. A single image can be reused across multiple nodes, including advanced situations such as deploying HA Kubernetes clusters and individualized network configuration per node. By leveraging a declarative, YAML-based definition format, EIB easily fits into existing GitOps and CI/CD pipelines, generating reproducible images with full accountability of their contents.

=== EIB Config

==== Management Cluster EIB Definition

The following steps are required to generate the EIB .iso image for the mgmt cluster:

 . Generate the folder structure with the following content in the eib-image folder following
the structure and files content described in https://documentation.suse.com/suse-edge/3.4/html/edge/atip-management-cluster.html#[official docs]:
...
└── mgmt-cluster-network.yaml
 ...
 . Add your KIWI iso image (output from kiwi process) as a base-image in the base-
images folder. To do that you can follow the steps in the https://documentation.suse.com/suse-edge/3.4/html/edge/guides-kiwi-builder-images.html[full guide instructions in official docs]

 . Replace from the original documentation the following placeholders with the values:

.. In kubernetes/helm/values/metal3.yaml file, change the ironicIP with your mgmt
   cluster IP address (e.g., 172.30.8.11)
.. In kubernetes/helm/values/rancher.yaml file, change the hostname with your mgmt
   cluster IP address (e.g., 172.30.8.11)
.. In network/mgmt-cluster-network.yaml file, change the network settings according
   to your environment (e.g., 172.30.8.11, netmask, gateway, dns and mac):
...
interfaces:
 ...
 . Modify the eib/mgmt-cluster-34-61-defaultNet.yaml file to change the following:

 ----
- ${ROOT_PASSWORD} - The root password for the management cluster. This could be
generated using `openssl passwd -6 PASSWORD` and replacing PASSWORD with the desired
password, and then replacing the value in the file. The final Rancher password will be
configured based on the file `custom/files/basic-setup.sh`.
- ${SCC_REGISTRATION_CODE} - The registration code for the SUSE Customer Center for the
SLE Micro product. This could be obtained from the SUSE Customer Center and replacing
the value in the file.
 ----

 . Build the eib image using the following command:
...
The following steps are required to generate the EIB .raw file for downstream clusters:

 . The folder `eib-edge-34-61` contains all the necessary files to generate the EIB image for downstream clusters with some telco packages and performance scripts. You can get a copy of the content of files using the following link from SUSE Telco Cloud official docs: https://documentation.suse.com/suse-edge/3.4/html/edge/atip-automated-provisioning.html# [Automated Provisioning – SUSE Telco Cloud Official Docs].
...
`-- network
 ...
 . Add your KIWI iso image (output from kiwi process) as a base-image in the base-images folder. To do that you can follow the steps in the https://documentation.suse.com/suse-edge/3.4/html/edge/guides-kiwi-builder-images.html[full guide instructions in official docs]

 . From the configuration file *edge-cluster.yaml* you can see the packages and scripts that will be included in the image. You will need to modify the following parameters according to your environment:


- encryptedPassword: Password for the root user in the image (you can generate it using `openssl passwd -6`)
- sccRegistrationCode: SCC registration code to register the system (if you have one)
...
== Deployment Details
 ...
 . Deployment Checklist for Telco Operators

    .. Performance & Sizing
        *** Profile RAN, Core, and Edge workloads (packet rate, concurrency, inference rate)
        *** Select Intel® Xeon® 6 SoC 6 SKUs based on core density and memory requirements
    ...
    .. Operational Deployment
        ...
        *** Test failover, redundancy and RAN real-time behaviour
        *** Align lifecycle refresh with OEM roadmaps]

=== SUSE Telco Cloud Direct Network provisioning

==== Intro
https://documentation.suse.com/suse-edge/3.4/html/edge/atip-automated-provisioning.html[Directed network provisioning] is a feature that allows you to automate the provisioning of downstream clusters. This feature is useful when you have many downstream clusters to provision, and you want to automate the process.

==== Cluster API & SUSE Telco Cloud
https://documentation.suse.com/suse-edge/3.4/html/edge/quickstart-metal3.html#id-high-level-architecture-2[Cluster API (CAPI)] enables management of infrastructure resources across multiple infrastructure providers via broadly adopted vendor-neutral APIs.

==== Management Cluster Deployment

 - Load the Management cluster EIB .iso generated previously in the Supermicro BMC:

image::media/src/png/8.1.3-load-management-cluster.png[align="center",width=90%]

 - Boot the server from Virtual Media CD

image::media/src/png/8.1.3-boot-1.png[align="center",width=90%]

image::media/src/png/8.1.3-boot-2.png[align="center",width=90%]

 - The installation will start automatically without any manual interaction. After having the operating system installed, the system will start configuring all parameters from the operating system as well as the RKE2 cluster with all helm charts required. No manual interaction is required to install the management cluster:

image::media/src/png/8.1.3-installation-1.png[align="center",width=90%]

image::media/src/png/8.1.3-installation-2.png[align="center",width=90%]

image::media/src/png/8.1.3-installation-3.png[align="center",width=90%]

image::media/src/png/8.1.3-installation-4.png[align="center",width=90%]

 - The system is ready and the management cluster has been installed properly. Kubeconfig is already loaded by default with the user provided in EIB definition file so we can double check cluster is ready:
----
mgmt-cluster-network:~ # k get nodes
NAME                   STATUS   ROLES                       AGE   VERSION
mgmt-cluster-network   Ready    control-plane,etcd,master   37s   v1.33.3+rke2r1
----
 - Rancher also is available and we can double check it. To get access to the Rancher UI, open a web browser and navigate to `https://rancher-172.30.8.11.sslip.io:8443/dashboard/home` and use the credentials configured in the file `custom/files/basic-setup.sh  (RANCHER_FINALPASSWORD)` during the EIB process showed before:

image::media/src/png/8.1.3-rancher-ui-1.png[align="center",width=90%]

image::media/src/png/8.1.3-rancher-ui-2.png[align="center",width=90%]

==== Downstream Cluster Deployment (CAPI + Metal3)
To deploy the downstream cluster, we need to create some manifests files from the management cluster already deployed (also could be done with RancherUI) in order to define the BaremetalHost object (bmh.yaml file) and the provision file (capi.yaml) to deploy the operating system and provision the RKE2 cluster automatically from the management cluster. To deploy the downstream cluster, it is necessary to create several manifest files from the already deployed management cluster (alternatively, this can be accomplished via the Rancher UI). These files include the BaremetalHost object definition (bmh.yaml) and the provision file (capi.yaml), which are required to deploy the operating system and automatically provision the RKE2 cluster from the management cluster.

 - Create the BaremetalHost file (bmh.yaml) replacing the BMC username/password as well as the MAC address and BMC IP in the URL:
...
....
 ...
 - Same info is represented now in Rancher UI:

image::media/src/png/8.1.4-rancher-ui-1.png[align="center",width=90%]

image::media/src/png/8.1.4-rancher-ui-2.png[align="center",width=90%]

 - Apply the capi manifests (capi.yaml) to provision the host with the operating system, RKE2 cluster as well as all Telco capabilities like kernel args, sriov, cpuisolation, etc.
...
----
 - Also, the same info is available now in Rancher UI:

image::media/src/png/8.1.4-rancher-ui-3.png[align="center",width=90%]

image::media/src/png/8.1.4-rancher-ui-4.png[align="center",width=90%]

== Performance Testing

=== Intro

We are conducting performance tests to measure the CPU latency in scheduling new tasks on both isolated and housekeeping cores. We utilize a Real-Time application, which is a simulator, to replicate specific workloads per CPU during defined time periods. By configuring a JSON file, we can simulate a particular CPU load percentage and use cycletest to measure the latency associated with scheduling tasks concurrently. Cycletest quantifies the latency before and after the task scheduling, subsequently generating a plot based on a resulting histogram.cat
...
Based on the following configurations:
 ...
 - Performance settings scripts showed in the capi.yaml and official documentation (https://documentation.suse.com/suse-edge/3.4/html/edge/atip-automated-provisioning.html#add-telco)
...
image::media/src/png/9.2-latency-plot.png[align="center",width=90%]
...
To move forward with transforming your network, please contact your SUSE, Intel, and Supermicro account teams to arrange a joint solution workshop and receive expert sizing assistance tailored to your specific deployment needs. For technical deep dives, you can access the SUSE Telco Cloud deployment documentation (https://documentation.suse.com/suse-edge/3.4/html/edge/atip.html), review Intel’s resources on the Xeon® 6 Processors Networking and Edge(https://www.intel.com/content/www/us/en/products/details/processors/xeon/edge.html), the Platform and Performance Advantages to Accelerate vRAN Deployments(https://www.intel.com/content/dam/www/central-libraries/us/en/documents/2025-02/vran-xeon6-soc-p-cores-solution-brief-final.pdf), and the Leadership AI and Networking Solutions with Xeon 6 Processors(https://www.intc.com/news-events/press-releases/detail/1728/intel-unveils-leadership-ai-and-networking-solutions-with), and examine the Supermicro specifications for the link:{sys112D-website}[`{sys112D}`] and {sys212D-website}[`{sys212D}`] systems.
...
This validated reference architecture confirms that SUSE Telco Cloud running on Supermicro telco-optimized systems powered by Intel Xeon 6 SoC processors can deliver a fully consolidated, CPU-only telco edge platform capable of supporting AI-native, low-latency 5G workloads at scale. The solution was validated to meet stringent telco requirements, demonstrating deterministic sub-5 microsecond latency under high CPU utilization while consolidating vRAN acceleration, AI inference, networking, timing, and security into a single-socket system.
