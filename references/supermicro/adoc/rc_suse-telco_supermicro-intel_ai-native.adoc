...
images folder.
To do that you can follow the steps in the {suse-linux-micro-image-with-kiwi}[full guide instructions in official docs]
...
The following steps are required to generate the EIB .raw file for downstream clusters:
 ...
 . The folder *eib-edge-34-61* contains all the necessary files to generate the EIB image for downstream clusters with some telco packages and performance scripts.
 You can get a copy of the content of files using the following link from SUSE Telco Cloud official docs: {suse-telco-automated} [Automated Provisioning – SUSE Telco Cloud Official Docs].
+
[listing]
----
.
...
----
 ...
 . Add your KIWI iso image (output from kiwi process) as a base-image in the base-images folder. To do that you can follow the steps in the {suse-linux-micro-image-with-kiwi}[full guide instructions in official docs]
...
==== Intro
{suse-telco-automated}[Directed network provisioning] is a feature that allows you to automate the provisioning of downstream clusters.
...
{suse-telco-high-level-arc}[Cluster API (CAPI)] enables management of infrastructure resources across multiple infrastructure providers via broadly adopted vendor-neutral APIs.
...
image::media/src/png/8.1.3-boot-2.png[align="center",width=90%]
 ...
 - The installation will start automatically without any manual interaction.
 After having the operating system installed, the system will start configuring all parameters from the operating system as well as the RKE2 cluster with all helm charts required.
 No manual interaction is required to install the management cluster:
...
image::media/src/png/8.1.3-installation-4.png[align="center",width=90%]
 ...
 - The system is ready and the management cluster has been installed properly.
 Kubeconfig is already loaded by default with the user provided in EIB definition file so we can double check cluster is ready:
+
----
...
To deploy the downstream cluster, we need to create some manifests files from the management cluster already deployed (also could be done with RancherUI) in order to define the BaremetalHost object (bmh.yaml file) and the provision file (capi.yaml) to deploy the operating system and provision the RKE2 cluster automatically from the management cluster.
To deploy the downstream cluster, it is necessary to create several manifest files from the already deployed management cluster (alternatively, this can be accomplished via the Rancher UI).
These files include the BaremetalHost object definition (bmh.yaml) and the provision file (capi.yaml), which are required to deploy the operating system and automatically provision the RKE2 cluster from the management cluster.
...
For technical deep dives, you can access the {suse-telco-web}[SUSE Telco Cloud deployment documentation], review Intel’s resources on the {xeon-processor-networking}[Xeon® 6 Processors Networking and Edge], the {vRAN-platform}[Platform and Performance Advantages to Accelerate vRAN Deployments], and the {intel-pressrelease}[Leadership AI and Networking Solutions with Xeon 6 Processors], and examine the Supermicro specifications for the {smss112-website}[{smss112}] and {smss212-website}[{smss212}] systems.
