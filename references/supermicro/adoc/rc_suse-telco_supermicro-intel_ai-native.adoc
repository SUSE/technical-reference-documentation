...
// This step is confusing, since this is the first mention of a KIWI ISO image and no real instructions on how to generate it.  It also begs the question of why the SL Micro image needed to be downloaded.  Clarification is needed here.
. Add your {kiwi-docs}[{kiwi-long}] ISO image to the `base-images` directory.
//
+
For further guidance, see {stelco-docs-kiwi-builder}[Building updated SUSE Linux Micro images with {kiwi}]
...
+
--
* pass:[${ROOT_PASSWORD}] - The root password for the management cluster
//
+
This can be generated with `openssl passwd -6 PASSWORD`, replacing `PASSWORD` with your desired
password.
The final {rancher-short} password will be configured from the file, `custom/files/basic-setup.sh`.

* pass:[${SCC_REGISTRATION_CODE}] - Your registration code for {slm-long} from the {scc-website}[{scc-long}]
--
...
----
+
You should see the output like the following as the eib image is built:
+
[listing]
----
...
The following steps are required to generate the EIB RAW image for downstream clusters.
...
//
+
You can get a copy of the file contents at {stelco-docs-automated}[Fully automated directed network provisioning].
+
[listing]
----
.
...
. From the configuration file, `edge-cluster.yaml`, you can see the packages and scripts that will be included in the image.
//
...
* *sccRegistrationCode* - Your {scc} registration code to register the system
--
...
The following is a deployment checklist for telco operators:
...
+
image::8.1.3-load-management-cluster.png[Load the management cluster EIB ISO, align="center", width=90%]

. Boot the server from Virtual Media CD.
+
image::8.1.3-boot-1.png[Power on the server, align="center", width=90%]
+
image::8.1.3-boot-2.png[Boot the ISO image, align="center", width=90%]

. The installation starts automatically without any manual interaction.
After installing the operating system, the system starts configuring all parameters for the operating system and RKE2 cluster with all required Helm charts.
This process is illustrated in the following screenshots.
+
image::8.1.3-installation-1.png[Booting the installer, align="center",width=90%]
+
image::8.1.3-installation-2.png[Loading the {slm} RAW image, align="center",width=90%]
+
image::8.1.3-installation-3.png[Installing components, align="center", width=90%]
+
image::8.1.3-installation-4.png[Completing the installation, align="center", width=90%]

. At this stage, the system is ready and the management cluster has been installed properly.
Kubeconfig is loaded with the user provided in EIB definition file by default.
...
.. Access the {rancher} UI by open your Web browser to https://rancher-172.30.8.11.sslip.io:8443/.

.. Log in with the credentials configured in the file, `custom/files/basic-setup.sh`,  during the EIB process.
+
image::8.1.3-rancher-ui-1.png[{rancher} UI login screen, align="center", width=90%]
+
image::8.1.3-rancher-ui-2.png[{rancher} UI dashboard, align="center", width=90%]
