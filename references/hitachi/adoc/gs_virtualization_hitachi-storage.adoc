...
To configure and start multipathd, you must log in to each {svirt} node and perform the following operations on the command line:
...
.. To make the `/etc/multipath.conf` persistent after node restart, Create a CloudInit resource using `multipathd_hitachi.yaml` this will create `/etc/multipath.conf` on all nodes after restart.
//
+
[source, yaml]
----
      ...
      network:
        ...
        - name: "Start multipathd service"
          systemctl:
            ...
            start:
              - multipathd
  filename: multipathd_hitachi.yaml
  matchSelector: {}
----

.. Deploy CloudInit configuration with the following command.
+
[source, console]
----
kubectl apply -f multipathd_hitachi.yaml
----
