...
This document provides technical guidance for integrating {svirt} and {hitachivsp} using the {hspc-long}.
...
+
{svirt}’s {svirt-docs-csi}[CSI-based design] enables seamless integration with enterprise SANs like Hitachi VSP.
...
Production environments should feature at least three controller/worker nodes and a dedicated storage network.


. If you have not already done so, install {svirt}.
//
+
You can access the {svirt} {svirt-docs}[documentation] for {svirt-docs-iso}[step-by-step installation guidance], hardware and network requirements, and further technical details.
+
[TIP]
====
...
To configure and start multipathd, you must log into each {svirt} node and perform the following operations on the command line:
...
Integration of {hitachivsp} involves deployment of the {hspc} driver into your {svirt} environment.
...
=== Create a Secret with {hitachivsp} credentials

Provide your {hitachivsp} credentials to the {svirt} environment as follows.
...
This provides a {hspc} CSI StorageClass that can be used for PersistentVolumeClaims.
...
. Create `hitachi-pvc-50g.yaml`, specifying the storage capacity such as 50GiB, as follows:
+
[source, yaml]
----
apiVersion: v1
...
Attach the PVC to a pod to verify that that storage is accessible.
...
. Define a VolumeSnapshot resource referencing the PVC.
+
[source, yaml]
----
apiVersion: snapshot.storage.k8s.io/v1
...
== Validate {hitachivsp} storage with a VM in {svirt}
