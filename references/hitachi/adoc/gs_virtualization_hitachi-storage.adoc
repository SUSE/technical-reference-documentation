...
.. {svirt} nodes use {slmicro-long}, an immutable operating system, so you need a way to make the multipathd service and configuration persistentent across reboots.
You accomplish this with CloudInit by following these steps:

... Log into one node.

... Create the CloudInit resource file, `/oem/99-multipathd_hitachi.yaml`, with the following contents:
+
[source, yaml]
----
...
... Copy the `/oem/99-multipathd_hitachi.yaml` file to the `/oem` directory on each of the other nodes.

... Deploy the CloudInit resource from one of the nodes.
+
[source, console]
----
kubectl apply -f /oem/multipathd_hitachi.yaml
----
