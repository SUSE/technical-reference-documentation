...
The author wishes to acknowledge contributions from {hitachivsp-provider} engineering teams for validation and joint testing of the {hspc-long} integration with {svirt}.
...
{slmicro}'s reduced attack surface, immutable filesystem, and transactional updates with system rollbacks make it an ideal host for Kubernetes.
...
{empty} +
After implementing this integration in your {svirt} environment, you will be able to deploy virtual machines and containers backed by {hitachivsp} storage.
...
=== Define a Kubernetes Secret with {hitachivsp} credentials
...
This provides a {hspc} CSI {suse-terms-storageclass}[StorageClass] that can be used for {suse-terms-pvc}[PersistentVolumeClaims].
