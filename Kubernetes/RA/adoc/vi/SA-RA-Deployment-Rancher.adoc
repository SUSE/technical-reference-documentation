...
+
ifdef::BP[]
TIP: This process deploys an auto-generated, self-signed security certificate for the {pn_Rancher} service. Other <<g-security>> certificate authority options are described in the "Install/Upgrade {portfolioName} on a Kubernetes Cluster" in the {pn_Rancher} {pn_Rancher_DocURL}[product documentation].
endif::BP[]
+
