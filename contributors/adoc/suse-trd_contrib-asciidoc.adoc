...
Block elements (or blocks) are discrete chunks of content that form the basic structure of an AsciiDoc document.
An obvious example of a block element is a source code block.
Other examples include paragraphs, lists, image macros, tables, and sections.
The document itself is even considered a block.

Blocks are defined using some form of line-oriented syntax.
Paragraphs, for example, are a contiguous set of lines of text.
Code blocks are enclosed in matching delimiter lines (typically, four hyphens).
List items are preceded by a prefix character (such as an asterisk or period) and continue until a blank line is reached.
