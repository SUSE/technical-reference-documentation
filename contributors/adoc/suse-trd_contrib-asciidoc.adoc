...
Whether implied by syntax or explicitly declared, how a block is processed and rendered is determined by its content model, context, and style.

This guide provides an overview of syntax for common block types.
To learn more blocks and block characteristics, see {asciidoc-docs-blocks}[AsciiDoc Docs: Blocks].
...
Comment block::
Designate several lines as a commentary by delimiting it with four forward slashes ( `////` ):
+
[source, asciidoc]
----
////
