...
Some other block elements are sections, lists, tables, image macros, listing blocks, source blocks, and admonitions.

When describing procedures in technical documentation, it is

{asciidoc-docs-continuation}[List continuation] refers to attaching a block to a list item


is explicitly denoted by a plus symbol ( `+` ), appearing by itself on the line between two blocks.
 can be used to join two adjacent blocks together.
