...
This section provides an introduction to AsciiDoc with examples to help you get started.
...
[source, asciidoc]
-----
. List the versions of `kernel-default` installed on your system.
[source, console]
----
...
You can proceed to continue the list item with as many block elements as required.
For example:
...
[listing]
----
kernel-default-7.0.5-1.1.x86_64
kernel-default-7.0.10-2.1.x86_64
----
+
[TIP]
====
Learn more about the `rpm` command with:
...
Notice that the and `IMPORTANT` admonition are correctly aligned under the list item, but the `TIP` admonition is aligned with the document margin.
As a bonus, there is a source block inside and properly aligned with `TIP` admonition.
