...
And, for source code, it may involve syntax highlighting with color or other text styles.
...
[IMPORTANT]
====
In this example, the command source block and its output in the listing block are embedded in an ordered list item.
This is accomplished by appending the preceding line with a space and plus symbol (` +`).
...
And, if you ever had to update the argument in a future revision of your document, you would have to do so everywhere it appears.

This is a perfect use for a variable (document attribute).
However, listing and source code blocks are verbatim.
So AsciiDoc directives and document attributes are ignored inside them.

Fortunately, AsciiDoc provides a mechanism in the form of customized substitutions.
This is accomplished by setting the `subs` attribute for a block.

Consider the simple example below.

In your vars file, define the variable you wish to use in a command example:
...
In your main content file, you then reference it inside a code block:
...
As in this example, `subs="attributes+"` is the most common substitution in {trd-alt}.
You can learn about others in {asciidoc-docs}subs/apply-subs-to-blocks/[AsciiDoc Docs: Customize the Substitutions Applied to Blocks].


[CAUTION]
====
Attribute substitution can be tricky.
The AsciiDoc rendering engine has no way of knowing if curly braces enclosing text in your code should be treated literally or substituted with the value of a variable you have defined.
...
However, if attribute substitutions are enabled, the rendering engine will try to find a variable, named `myVariable`, and substitute its value.
If there is no defined variable, the render would fail.
